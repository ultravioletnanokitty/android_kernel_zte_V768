...
#if (defined(DSI_USE_DYN_AHB_CLK) )
#define CSL_DSI_ENA_AHB_CLK()  clk_enable(clk_get(NULL, BCM_CLK_MIPIDSI_STR_ID)); \
                                                      clk_enable(clk_get(NULL, BCM_CLK_USB_PHY_REF_STR_ID)); 
#define CSL_DSI_DIS_AHB_CLK()  clk_disable(clk_get(NULL, BCM_CLK_MIPIDSI_STR_ID)); \
                                                      clk_disable(clk_get(NULL, BCM_CLK_USB_PHY_REF_STR_ID)); 

#define SYSCFG_DSICR 0x08880038
#define CLK_ANALOG_PHASES_ENABLE  0x08140140

#define CSL_DSI_ENA_AHB_CLK_PIO()  writel(1, IO_ADDRESS(SYSCFG_DSICR));  \
                                                      		writel(readl(IO_ADDRESS(CLK_ANALOG_PHASES_ENABLE)) &~(0x00000002), IO_ADDRESS(CLK_ANALOG_PHASES_ENABLE)); 

#define CSL_DSI_DIS_AHB_CLK_PIO()  writel(0, IO_ADDRESS(SYSCFG_DSICR));  \
                                                      	      writel(readl(IO_ADDRESS(CLK_ANALOG_PHASES_ENABLE)) & 0xFFFFFFFF, IO_ADDRESS(CLK_ANALOG_PHASES_ENABLE)); 
                                
#else
#define CSL_DSI_ENA_AHB_CLK()
#define CSL_DSI_DIS_AHB_CLK()
#endif
...
static CSL_LCD_RES_T cslDsiDmaStart ( DSI_UPD_REQ_MSG_T* updMsg )
{
    ...
    updMsg->dmaChInfo.dstMaster = 1;
    updMsg->dmaChInfo.srcMaster = 1;
...
/* ***************************************************************************** */
/* Function Name:  CSL_DSI_Lock_NoSem*/
/* Description: */
/* ***************************************************************************** */
CSL_LCD_RES_T CSL_DSI_Lock_NoSem(CSL_LCD_HANDLE vcH )
{
    DSI_CLIENT          clientH;
...
/* ***************************************************************************** */
/* Function Name:  CSL_DSI_Unlock_NoSem */
/* Description: */
/* ***************************************************************************** */
CSL_LCD_RES_T CSL_DSI_Unlock_NoSem(CSL_LCD_HANDLE vcH )
{
    DSI_CLIENT          clientH;
    DSI_CM_HANDLE       dsiChH;
...
/* yaogangxiang@wind-mobi.com 2012.07.03 begin */
//1.before entering system,there will be flicker
//2.in Camera,there will be flicker
//CSP#523614
#define DSI_FIFO_SIZE_BYTES (64 * 4)
...
//    pktSizeBytes = (req->lineLenP * dsiChH->bpp_wire);
    int frameSizeBytes = req->lineLenP * req->lineCount * dsiChH->bpp_wire;
    pktSizeBytes = DSI_FIFO_SIZE_BYTES / dsiChH->bpp_dma * dsiChH->bpp_wire;
...
//    pktCount = req->lineCount;
    pktCount = frameSizeBytes / pktSizeBytes;
...
//    pktWc = req->lineLenP >> dsiChH->wc_rshift;
    pktWc = (pktSizeBytes / dsiChH->bpp_wire) >> dsiChH->wc_rshift;
    if( !clientH->hasLock ) OSSEMAPHORE_Obtain ( dsiH->semaDsi, TICKS_FOREVER );
    ...
    /*The packet doesn't get scheduled if  packet size < DSI_DMA_THRE value */
/*    if((pktSizeBytes / 4) < 48)
       chal_dsi_de1_set_dma_thresh ( dsiH->chalH, (pktSizeBytes / 4));
*/
    if((pktSizeBytes / dsiChH->bpp_wire) < 48)
       chal_dsi_de1_set_dma_thresh ( dsiH->chalH, (pktSizeBytes /
           dsiChH->bpp_wire));
    else
       chal_dsi_de1_set_dma_thresh ( dsiH->chalH, 48);

/* yaogangxiang@wind-mobi.com 2012.07.03 end */     
    updMsgCm.updReq  = *req;
    updMsgCm.dsiH    = dsiH; 
...
#define MAX_SIZE_ONE_PACKET 64
 ...
 //   unsigned int* src_buffer = (unsigned int*)phys_to_virt((req->buff));
    ...
    if( !clientH->hasLock ) 
    	...
    	{ CSL_DSI_ENA_AHB_CLK_PIO(); }

    chal_dsi_clr_status ( dsiH->chalH, 0xFFFFFFFF );

    /* PIO mode of operation requires that the threshold be set to 0 always*/
    chal_dsi_de1_set_dma_thresh ( dsiH->chalH, 0);
    ...
    while( transferWc > 0 )
    {
		...
		chal_dsi_update_pixel_fifo(dsiH->chalH,data_buff, pktWc);
		// send first line with START CMND
		chal_dsi_clr_status ( dsiH->chalH, 0xFFFFFFFF );
		chal_dsi_de1_send ( dsiH->chalH, TX_PKT_ENG_0, &dsiChH->chalCmCfg );

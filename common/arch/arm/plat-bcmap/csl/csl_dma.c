...
Int32 csl_dma_obtain_channel(UInt8 srcID, UInt8 dstID)
{
	...
	if (srcID == DMA_CLIENT_AUDIO_WB_MIXERTAP) {
		...
		chan = DMA_CHANNEL_1;
//#if defined (CONFIG_BOARD_PASCAL) || defined (CONFIG_BOARD_L400)
#if 0
/* yaogangxiang@wind-mobi.com 2011.12.30 begin */
/* broadcom for LCD mipi interface just temporary */
	} else if (dstID == DMA_CLIENT_LCD || dstID == DMA_CLIENT_DSI_CM) {
		chan = DMA_CHANNEL_1;
#endif
/* yaogangxiang@wind-mobi.com 2011.12.30 end */
	} else
#endif
	{

...
int32_t __init bcm_gpio_init(struct bcm_gpio_port *port)
{
	...
	for (i = GPIO_TO_IRQ(0); i < GPIO_TO_IRQ(ARCH_NR_GPIOS); i++) {
		...
		//feixiaoping@wind-mobi.com 2012-0725 begin
		//modify wifi interrupt trigger way as dege mode
		//prevent wifi can't wake up for losing the interrupt
		#if 0
		...
		if(i==GPIO_TO_IRQ(18))
		{
			//printk("------bcm_gpio_init--->Set GPIO 18 IRQ in edge mode---\n");
		...
		#endif
		//feixiaoping@wind-mobi.com 2012-0725 end

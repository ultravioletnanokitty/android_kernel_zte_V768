...
#if defined(CONFIG_ZTE_CDROM_SUPPORT)
...
/* yuanlan@wind-mobi.com 2012.08.16begin */
/* Add synaptics Capacitive touch panel support */
#ifdef CONFIG_TOUCHSCREEN_SYNAPTICS_I2C_RMI
#include <linux/input/synaptics_rmi.h>
#endif
/* yuanlan@wind-mobi.com 2012.08.16 end */
...
#ifdef CONFIG_BOARD_PASCAL_TYPE_EDN10
...
struct tmd2771x_platform_data tmd2771x_pdata = {
	...
	//fix sensor is invalid
	//yuanlan@wind-mobi.com 2012-3-6 review
...
#if (defined (CONFIG_TOUCHSCREEN_MSI_ATMEGA168)|| defined (CONFIG_TOUCHSCREEN_FT5X06) || defined(CONFIG_TOUCHSCREEN_GT818) || defined(CONFIG_TOUCHSCREEN_MXT140) || defined (CONFIG_TOUCHSCREEN_SYNAPTICS_I2C_RMI))
...
/* yaogangxiang@wind-mobi.com 2012.09.18 begin */
static int mxt140_exit_platform_hw(void)
{
       gpio_free(PEN_IRQ_GPIO);
}
/* yaogangxiang@wind-mobi.com 2012.09.18 end */

static struct mxt140_platform_data mxt140_platform_data = {
	.i2c_pdata = {.i2c_spd = I2C_SPD_100K,},//I2C_SPD_400K
	.platform_init  = mxt140_init_platform_hw,
        .platform_exit = mxt140_exit_platform_hw,
	.x_line         = 15,
	.y_line         = 9,
	.x_size         = 520,
	.y_size         = 320,
...
static struct i2c_board_info __initdata i2c_gpio_devices[] = {  
 ...
 /* xiongbiao@wind-mobi.com 2011.12.13 end*/
  ...
  /* yuanlan@wind-mobi.com 2012.08.16 begin */
  /* Add synptics      Capacitive touch panel support */
#ifdef CONFIG_TOUCHSCREEN_SYNAPTICS_I2C_RMI
                 {
                  I2C_BOARD_INFO("synaptics_rmi4_i2c", 0x22),
                  .platform_data = (void *)&rmi4_platformdata,
                  .irq = GPIO_TO_IRQ(PEN_IRQ_GPIO),
                  },
...
//hanwei@wind-mobi.com 20111021 begin

#elif (defined(CONFIG_BCM_CAM_OV7692) && defined(CONFIG_BCM_CAM_HI253)) /*for Acar project*/
#define I2C_PRIMARY_CAM_SLAVE_ADDRESS          0x40   /*hi253*/
#define I2C_SECONDARY_CAM_SLAVE_ADDRESS                0x78   /*ov7692*/  //0x78:write   0x79:read 

struct i2c_slave_platform_data hi253_cam_pdata = {
        .i2c_spd = I2C_SPD_100K,
};

struct i2c_slave_platform_data ov7692_cam_pdata = {
        .i2c_spd = I2C_SPD_400K,
};

static struct i2c_board_info __initdata bcm21553_cam_i2c_board_info[] = {
        {I2C_BOARD_INFO("primary_cami2c", (I2C_PRIMARY_CAM_SLAVE_ADDRESS >> 1)),
         .platform_data = (void *)&hi253_cam_pdata,
         .irq = IRQ_CAM_CCP2,
         },

               {I2C_BOARD_INFO("secondary_cami2c", (I2C_SECONDARY_CAM_SLAVE_ADDRESS >> 1)),
         .platform_data = (void *)&ov7692_cam_pdata,
         .irq = IRQ_CAM_CCP2,
         },
};
//heweimao@wind-mobi.com 20120117 begin
//meger camera patch
#elif defined(CONFIG_BCM_CAM_MT9D115)/*for L301 MIPI project*/
#define I2C_PRIMARY_CAM_SLAVE_ADDRESS          0x78   /*MT9D115*/

//zhaoqiang@wind-mobi.com 20120329 begin
//modify i2c speed to 400K for decrease camera initialization time
struct i2c_slave_platform_data mt9d115_cam_pdata = {
        .i2c_spd = I2C_SPD_400K,
};
//zhaoqiang@wind-mobi.com 20120329 end

static struct i2c_board_info __initdata bcm21553_cam_i2c_board_info[] = {
        {I2C_BOARD_INFO("cami2c", (I2C_PRIMARY_CAM_SLAVE_ADDRESS >> 1)),
         .platform_data = (void *)&mt9d115_cam_pdata,
         .irq = IRQ_CAM_CSI2,
        },
};
//heweimao@wind-mobi.com 20120117 end
#else
;
 /*todo*/
...
static struct max8986_power_pdata power_pdata = {
	...
	.wac_charging_cc = MAX8986_CHARGING_CURR_700MA,
	.eoc_current = MAX8986_EOC_120MA,
...
#if defined(CONFIG_ZTE_CDROM_SUPPORT)
extern int Android_switch_usb_conf (unsigned char new_usb_conf);
#define MSC_CDROM_MODE 8
//feixiaoping@wind-mobi.com 2012.07.14  begin
// insert usb cable and power on then adb shell fail problem
extern int UmsCDEnable;
//feixiaoping@wind-mobi.com 2012.07.14  end
...
/* review by liubing */
      static u16 bat_vol[] = {3400, 3600, 3695, 3725, 3760, 3790, 3820, 3850, 3880, 3920, 3955, 4015, 4200};
	static u8 bat_per[] = {0, 16, 26, 32, 42, 50, 56, 62, 68, 76, 83, 92, 100};
#ifdef BATTERY_ADC_FIX
static u16 bat_vol_charging_usb[] = {3490, 3690, 3785, 3815, 3850, 3880, 3910, 3940, 3970, 4010, 4045, 4105, 4220};
static u16 bat_vol_charging_ac[] = {3530, 3740, 3835, 3865, 3900, 3930, 3960, 3990, 4020, 4060, 4095, 4140, 4245};
...
/* review by liubing */
        ...
        if(check_time_delta(read_time, boot_begin_time) < ADC_FIX_TIME)
        {
         ...
         if(pmu_get_charging_status()==POWER_SUPPLY_STATUS_CHARGING){
            ...
            u32 vol = ret - 4*bat_adc_volt_mul;
		if(max89xx_muic_get_charger_type() == PMU_MUIC_CHGTYP_USB)
		 {	bat_vol_charging = bat_vol_charging_usb;
            ...
            if(vol < bat_vol_charging[0]*1000){
                ...
                if(0){//check_time_delta(read_time,boot_begin_time)< STABLE_BOOT_TIME){
                     ret = param*bat_adc_volt_mul;
                }else{
		...
		//maxiaohui@wind-mobi.com 2012.05.20 end
			{
			    ...
			    //feixiaoping@wind-mobi.com 2012.07.14  begin
			    // insert usb cable and power on adb shell fail problem
                //UmsCDEnable initial value is 1;
               if(param == PMU_MUIC_CHGTYP_USB && UmsCDEnable == 0&& POWEROFF_CHARGING != get_ap_boot_mode())
               	{
			   		if(debug_mask)
						printk("------cdrom---pum_event_callback--pmu usb type = usb->UmsCDEnable=%d---\n",UmsCDEnable);
              	 	//feixiaoping@wind-mobi.com 2012.07.18  begin			    	
              	 	// exit cdrom then insert usb again you can't see the cdrom and you can't install the driver                	
              	 	//set  UmsCDEnable  value as 1;
			   		UmsCDEnable=1;
			   		Android_switch_usb_conf(MSC_CDROM_MODE);
               	}
...
static struct android_usb_platform_data android_usb_pdata = {
	...
	.vendor_id = 0x19D2,
	.product_id = 0x0358,
	.adb_product_id = 0x0355,	
#else
	.vendor_id = 0x18d1,//0x19D2,
	.product_id = 0x0005,//0x0083,
	...
	//liutao@wind-mobi.com 2012-7-13 begin
	//modify usb serial number
	//yuanlan@wind-mobi.com 2012-7-13 end
	.serial_number="0123456789ABCDEF",
	//liutao@wind-mobi.com 2012-7-13 end
	.nluns = 1,
};

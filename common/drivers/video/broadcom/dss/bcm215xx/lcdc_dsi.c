...
#if defined(CONFIG_BCM_LCD_HX8369A_TFT480800)
#include "../../displays/lcd_hx8369a_TFT480800.h"
#define CONFIG_ARGB8888
#elif defined(CONFIG_BCM_LCD_HX8369A_T43KPS06)
#include "../../displays/lcd_HX8369A_T43KPS06.h"
#define CONFIG_ARGB8888
#elif defined(CONFIG_BCM_LCD_HX8357C_GZ3513)//for HX8357 L301
#include "../../displays/lcd_HX8357C_GZ3513.h"
#define CONFIG_ARGB8888
#elif defined(CONFIG_BCM_LCD_HX8363B_YT40F26B0)//for HX8363 L400
#include "../../displays/lcd_HX8363B_YT40F26B0.h"
#elif defined(CONFIG_BCM_LCD_HX8369A_YT40F25J0)
#include "../../displays/lcd_HX8369A_YT40F25J0.h"
#define CONFIG_ARGB8888
//#endif 
//xiongbiao@wind-mobi.com 2012.02.09 begin
//support mipi lcd compatible on L301
#elif defined(CONFIG_LCD_DSI_DRV_ALL)//for all L301
#include "../../displays/lcd_HX8357C_GZ3513_compatible.h"
#include "../../displays/lcd_HX8357C_T35JKS18C1TP_compatible.h"
#include "../../displays/lcd_HX8357C_TFT8K7692_compatible.h"
#include "../../displays/lcd_HX8357C_T35JKS18C1TP_compatible_green.h"
#define CONFIG_ARGB8888
//xiongbiao@wind-mobi.com 2012.02.09 end
//xiongbiao@wind-mobi.com 2012.02.20 begin
//support mipi lcd compatible on L400
#elif defined(CONFIG_LCD_L400_DSI_DRV_ALL)//for all L301
#include "../../displays/lcd_HX8363B_YT40F26B0_compatible.h"
#include "../../displays/lcd_HX8369A_YT40F25J0_compatible.h"
#include "../../displays/lcd_HX8369A_TM040YDZ01_compatible.h"
#include "../../displays/lcd_NT35510_4014TN_compatible.h"
#include "../../displays/lcd_HX8369A_BT040TN04_compatible.h"
...
//xiongbiao@wind-mobi.com 2012.02.09 begin
//support mipi lcd compatible on L301
//for TRULY.ltd's panel
#if defined(CONFIG_LCD_DSI_DRV_ALL)
lcd_dsi_drv_info_t* lcd_drv[] = {
	&lcd_hx8357c_gz3513,
	&lcd_hx8357c_t35jks18c1tp_green,//lcd_hx8357c_t35jks18c1tp(old lcd  driver)->lcd_hx8357c_t35jks18c1tp_green(new lcd driver)
	&lcd_hx8357c_tft8k7692,//lcd_hx8357c_tft8k7692->truly lcd driver
	&lcd_hx8357c_gz3513,
	0,
};
lcd_dsi_drv_info_t* cur_lcd_dsi_drv_ptr = &lcd_hx8357c_gz3513;

extern LCD_dev_info_t* LCD_device;          

#define PANEL_HEIGHT            cur_lcd_dsi_drv_ptr->lcd_dev->height
#define PANEL_WIDTH             cur_lcd_dsi_drv_ptr->lcd_dev->width
...
lcd_dsi_drv_info_t* lcd_drv[] = {
	&lcd_hx8369a_bt040tn04,//feixiaoping@wind-mobi.com 2012.04.05 add BaoLongDa lcd config
	&lcd_hx8363b_yt40f26b0,//YASHI 
	&lcd_hx8369a_tm040ydz01,//tianma
	&lcd_hx8369a_yt40f25j0,//IPS
	0,
};
lcd_dsi_drv_info_t* cur_lcd_dsi_drv_ptr = &lcd_hx8363b_yt40f26b0;
...
//support mipi lcd compatible on L301
	...
	cur_lcd_dsi_drv_ptr->dsi_cfg->bus = busId;
#else
    dsiCfg.bus = busId;
#endif
//xiongbiao@wind-mobi.com 2012.02.09 end
...
//support mipi lcd compatible on L301
#if defined(CONFIG_LCD_DSI_DRV_ALL)||defined(CONFIG_LCD_L400_DSI_DRV_ALL)
	if(cur_lcd_dsi_drv_ptr->dsi_cm_vc->teCfg.teInType != DSI_TE_NONE)
#else
...
//support mipi lcd compatible on L301
#if defined(CONFIG_LCD_DSI_DRV_ALL)||defined(CONFIG_LCD_L400_DSI_DRV_ALL)
	if ( CSL_DSI_Init( pInitCfg, cur_lcd_dsi_drv_ptr->dsi_cfg ) != CSL_LCD_OK )
#else
...
//support mipi lcd compatible on L301
#if defined(CONFIG_LCD_DSI_DRV_ALL)||defined(CONFIG_LCD_L400_DSI_DRV_ALL)
    if ( CSL_DSI_OpenCmVc ( pPanel->clientH, cur_lcd_dsi_drv_ptr->dsi_cm_vc, &pPanel->dsiCmVcHandle )
            != CSL_LCD_OK )
#else
...
//
//*****************************************************************************
static Int32 dsi_set_window ( DISPDRV_HANDLE_T drvH,LCD_dev_info_t* dev )
{
	dsic_panel_t *pPanel = (dsic_panel_t *)drvH;
	UInt8 dataptr[4];
    ...
    )
{
	...
	source = (UInt32)dev->frame_buffer.virtPtr + stride * dev->dirty_rect.top +
					column * INPUT_BPP;
...
#endif
#ifdef LCD_COLUMN_UPDATE_CPU
		data_buff = (uint32_t *)kzalloc(count * sizeof(unsigned int), GFP_KERNEL);
        if (!data_buff) {
...
static void dsi_get_interface_info(uint32_t* phy_addr, int* width, int* height, int* bpp)
{
...
#if defined(CONFIG_LCD_DSI_DRV_ALL)||defined(CONFIG_LCD_L400_DSI_DRV_ALL)
    if(lcd_id==1)
	exec_cmnd_list(display_hdl, TRUE, cur_lcd_dsi_drv_ptr->power_on_seq);
	else
	exec_cmnd_list(display_hdl, TRUE, cur_lcd_dsi_drv_ptr->exit_sleep_seq);
#else
...
//support mipi lcd compatible on L301
#if defined(CONFIG_LCD_DSI_DRV_ALL)||defined(CONFIG_LCD_L400_DSI_DRV_ALL)
	exec_cmnd_list(display_hdl, TRUE, cur_lcd_dsi_drv_ptr->enter_sleep_seq);
#else
...
{
	dsic_panel_t *pPanel = &panel[LCD_main_panel];
	CSL_DSI_Lock_NoSem(pPanel->dsiCmVcHandle);
}
static void dsi_unlock_csl_handle(void)
{
	dsic_panel_t *pPanel = &panel[LCD_main_panel];
	CSL_DSI_Unlock_NoSem(pPanel->dsiCmVcHandle);
}
	...
	)
{
	...
	UInt32 row_offset = (dev->width - img_width)/2;
	UInt32 column_offset = (dev->height- img_height)/2;
	...
	{
		fb_ptr += column_offset*dev->width+row_offset;
		for (i = 0, rle_count = 0; i <(img_width*img_height); i++) {
		       if (0 >= rle_count) {
		         ...
		         *fb_ptr++ = pixel_data;
		         current_size++;
		         if(current_size >= img_width)
		         {
		            fb_ptr += (dev->width-img_width);
		            current_size = 0;
		         }
		}
		...
		if ( CSL_DSI_CPU_UpdateCmVc ( pPanel->dsiCmVcHandle, &req , fb_ptr) != CSL_LCD_OK )
		{
			LCD_DBG ( LCD_DBG_ERR_ID, "[DISPDRV] %s: ERROR ret by "
				"CSL_DSI_UpdateCmVc\n\r", __FUNCTION__ );
			return;
		...
		pr_info("dsi_send_data bpp=%d is not supported\n", LCD_device[0].bits_per_pixel);		
	return;
}

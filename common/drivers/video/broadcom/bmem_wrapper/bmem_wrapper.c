...
//pengzhiping V3d 2012.04.26 begin
static int bmemkill_pid = 0;
static struct proc_dir_entry *bmemkill_proc_file;
//pengzhiping  V3d 2012.04.26 end
...
static int bmem_oom_on_failure(int bmem_to_be_freed)
{
    ...
    if ((bmem_freed_sum < bmem_to_be_freed) && (bmem_kill_curr_on_fail)) {
		...
		//pengzhiping@wind-mobi.com V3D 2012.04.26 begin
		bmemkill_pid = current->tgid;
		//pengzhiping@wind-mobi.com V3D 2012.04.26 begin
...
//pengzhiping@wind-mobi.com V3D 2012.04.26 begin
/*
* bmemkill_proc_get_status()
* Description : proc read callback to print the bmem kill status
*/
static int bmemkill_proc_get_status(char *page, char **start,
		   off_t off, int count, int *eof, void *data)
{
	char *curr = page;
	int len = 0;
	...
	down(&bmem_status_sem);
	BMEM_PROC_PRINT_D("last killed task", bmemkill_pid);
	up(&bmem_status_sem);
...
//pengzhiping@wind-mobi.com V3D 2012.04.26 end
...
int __init bmem_wrapper_init(void)
{
	...
	//pengzhiping@wind.mobi.com V3D 2012.04.26 begin
	bmemkill_proc_file = create_proc_entry("bmemkill", 0644, NULL);
	if (bmemkill_proc_file) {
		bmemkill_proc_file->data = NULL;
		bmemkill_proc_file->read_proc = bmemkill_proc_get_status;
		bmemkill_proc_file->write_proc = NULL;
		// bmem_proc_file->owner = THIS_MODULE;
	}
	else {
		KLOG_E("Failed creating bmemkill proc entry");
	}
	//pengzhiping@wind.mobi.com V3D 2012.04.26 end
...
void __exit bmem_wrapper_cleanup(void)
{
	...
	//pengzhiping@wind.mobi.com V3D 2012.04.26 begin
	if (bmemkill_proc_file) {
		remove_proc_entry("bmemkill", bmemkill_proc_file);
	} 
	//pengzhiping@wind.mobi.com V3D 2012.04.26 end

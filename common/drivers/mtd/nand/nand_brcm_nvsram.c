...
static struct nand_otp brcm_nand_otp[] = {
	{NAND_MFR_MICRON, 0x1000, 0xf800, NAND_MICRON_OTP},
	{NAND_MFR_HYNIX, 0x10000, 0x1f800, NAND_HYNIX_OTP},
...
static void brcm_nand_wait_until_ready(void)
{
	...
	while (time_before(jiffies, timeo)) {
		...
		/* Schedule out only if NOT in panic context */
...
/* Hynix NAND commands */
#define NAND_CMD_ENTER_OTP_HYNIX 0xB1	/* magic number to differentiate old cmd IDs*/
#define NAND_CMD_EXIT_OTP_HYNIX 0xB2		/* magic number to differentiate old cmd IDs*/
				...
				int column, int page_addr)
{
	...
	case NAND_CMD_ENTER_OTP_HYNIX:
		addr.cmd_phase_addr = ((uint32_t)bcm_nand_io_base) | 0x04 << NVSRAM_SMC_NAND_START_CMD;
		writel(0, addr.cmd_phase_addr);

		addr.cmd_phase_addr = ((uint32_t)bcm_nand_io_base) | 0x19 << NVSRAM_SMC_NAND_START_CMD;
		writel(0, addr.cmd_phase_addr);
	...
	case NAND_CMD_EXIT_OTP_HYNIX:
		
		addr.cmd_phase_addr |= 0x0 << NVSRAM_SMC_CMD_NUM_ADDR_CYCLE | 0x06 << NVSRAM_SMC_NAND_START_CMD;
		writel(0, addr.cmd_phase_addr);
...
static void micron_nand_read_get_feature_param(
		struct mtd_info *mtd,
		const int feat_addr,
		unsigned char *p1_data,
		unsigned char *p2_data,
		unsigned char *p3_data,
		unsigned char *p4_data)
{
	...
	/* Micron send CMD #EEh ADDR P1 P2 P3 P4 */
	chip->cmdfunc(mtd, NAND_CMD_GET_FEATURES, feat_addr, -1);

	/*
	 * Micron NAND: Wait for STATUS_READY;
	 * (tWB + tFEAT + tRR) max ~ approx 1.12us.
	 * But we give around 5us to be safe
	 */
	for (count = 5; count > 0; count--) {
		ndelay(1000);	/* 1 micro sec delay */

		/* Send CMD 70h and check for STATUS_READY */
		chip->cmdfunc(mtd, NAND_CMD_STATUS, -1, -1);
		status = chip->read_byte(mtd);
		if (status & NAND_STATUS_READY)
			break;
	...
	if (!count) {
		pr_err("%s: NAND_STATUS timed out for GET_FEATURES CMD\n",
			__FUNCTION__);
		/* Return frowning */
		return;
	}
	...
	/* Read parameters */
	*p1_data = brcm_nand_read_byte(mtd);
	*p2_data = brcm_nand_read_byte(mtd);
	*p3_data = brcm_nand_read_byte(mtd);
	*p4_data = brcm_nand_read_byte(mtd);

	/* Return grinning */
	return;
}

static void micron_nand_write_set_feature_param(
		struct mtd_info *mtd,
		const int feat_addr,
		const unsigned char p1_data,
		const unsigned char p2_data,
		const unsigned char p3_data,
		const unsigned char p4_data)
{
	struct nand_chip *chip = mtd->priv;
	int count;
	int status;
	...
	chip->cmdfunc(mtd, NAND_CMD_SET_FEATURES, feat_addr, -1);
	udelay(5);

	/* Write parameters */
	brcm_nand_write_byte(p1_data);
	brcm_nand_write_byte(p2_data);
	brcm_nand_write_byte(p3_data);
	brcm_nand_write_byte(p4_data);
	...
	if (!count) {
		pr_err("%s: NAND_STATUS timed out for SET_FEATURES CMD. status: %x"
...
void enter_otp_mode(struct mtd_info *mtd)
{
	struct nand_chip *chip = mtd->priv;
	int feat_addr;
	unsigned char p1_data, p2_data, p3_data, p4_data;

	if (config_nand_otp == NAND_MICRON_OTP) {
		feat_addr = 0x90; /* Feature Address for enabling OTP mode */

		/* Read the parameters */
		micron_nand_read_get_feature_param(mtd,	feat_addr,
				&p1_data, &p2_data, &p3_data, &p4_data);

		pr_debug("GET_FEATURES returned => feat_addr: %x,"
			"p1_data: %x, p2_data: %x,"
			"p3_data: %x, p4_data: %x\n",
			feat_addr, p1_data, p2_data, p3_data, p4_data);

		/* Clear and set first bit of P1 param for OTP mode */
		p1_data &= ~0x01;
		p1_data |= 0x01;

		/* Write the parameters */
		micron_nand_write_set_feature_param(mtd, feat_addr,
				p1_data, p2_data, p3_data, p4_data);

		/* Read back the parameters and sanity check */
		micron_nand_read_get_feature_param(mtd, feat_addr,
				&p1_data, &p2_data, &p3_data, &p4_data);
	...
	} else if (config_nand_otp == NAND_HYNIX_OTP) {
		feat_addr = 0;
		chip->cmdfunc(mtd, NAND_CMD_ENTER_OTP_HYNIX, feat_addr, -1);
	} else {
...
void exit_otp_mode(struct mtd_info *mtd)
{
	...
	if (config_nand_otp == NAND_MICRON_OTP) {
		...
		/* Clear first bit of P1 param to exit OTP mode */
		p1_data &= ~0x01;
		...
		feat_addr = 0;
		chip->cmdfunc(mtd, NAND_CMD_EXIT_OTP_HYNIX, feat_addr, -1);
	}
	else {
		/* Stubs for other flash chips */
	}

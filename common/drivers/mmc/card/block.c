...
#if 1
extern unsigned int g_check_sd_cnt,g_check_sd_cnt1,g_check_sd_cnt2,g_check_sd_cnt3; //debug only
#else
	...
	do {
		...
		if (!mmc_host_is_spi(card->host) && rq_data_dir(req) != READ) {
			...
			do {
				...
				err = mmc_wait_for_cmd(card->host, &cmd, 5);
				if (err||(g_check_sd_cnt1>10)) {
					printk(KERN_ERR "%s: error %d requesting status,%d,%d\n",
					       req->rq_disk->disk_name, err,g_check_sd_cnt1,g_check_sd_cnt3);
					goto cmd_err;
				}
				/*
				 ...
				 */
				g_check_sd_cnt1++;				
				g_check_sd_cnt2=cmd.resp[0];
				/* feixiaoping@wind-mobi.com 20120706 begin */
				//the normal result is 1,0x900,1;I delete this log for it appears too many times.
				if(0)
					printk("mmc_blk_issue_rq=%d,0x%x,%d\n",g_check_sd_cnt1,g_check_sd_cnt2,g_check_sd_cnt3);
				
				/* feixiaoping@wind-mobi.com 20120706 end */

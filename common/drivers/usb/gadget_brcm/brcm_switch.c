...
#if defined(CONFIG_ZTE_CDROM_SUPPORT)
unsigned char g_usb_config_mode = 0;
#endif
...
{
#ifdef CONFIG_USB_ANDROID
	Android_switch_usb_conf(ACM_ONLY_MODE-1);
#endif
}

#if defined(CONFIG_ZTE_CDROM_SUPPORT)
static void brcm_switch_msc_cdrom(void* data)
{
#ifdef CONFIG_USB_ANDROID
        Android_switch_usb_conf(MSC_CDROM_CONF-1);
#endif
}
#endif
...
{		
#ifdef CONFIG_USB_ANDROID
	Android_switch_usb_conf(ACM_OBEX_MODE-1);				
#endif
}
...
static ssize_t brcm_switch_write(struct file *file, const char *buffer, unsigned long count, void *data)
{
	...
	case ACM_ONLY_MODE:
        ...
        case MSC_CDROM_CONF:
                        g_usb_config_mode = 9;
                        pr_info("\MSC_CDROM_CONF\n");
                        brcm_switch_task = kthread_run(brcm_switch_msc_cdrom, 0, "msc_cdrom");
                        break;
#endif			
	case ACM_OBEX_MODE:

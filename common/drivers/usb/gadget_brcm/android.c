...
/*-------------------------------------------------------------------------*/
#ifndef CONFIG_ZTE_CDROM_SUPPORT
#define RNDIS_VENDOR_NUM       0x0A5C  /* Broadcom */
#define RNDIS_PRODUCT_NUM      0xABCD  /* Ethernet/RNDIS Gadget */
#else
#define RNDIS_VENDOR_NUM        0x19d2//0x0A5C  /* Broadcom */
#define RNDIS_PRODUCT_NUM      0x356//0xABCD  /* Ethernet/RNDIS Gadget */
#endif
...
#define CDROM_PRODUCT_ID                          0x0005
#endif
#define ADB_PRODUCT_ID				0x0002
#else
#define VENDOR_ID					0x19d2//0x18D1
#define MSC_PRODUCT_ID				0x0391//0x0005
#if defined(CONFIG_ZTE_CDROM_SUPPORT)
#define CDROM_PRODUCT_ID                          0x0358//0x0005
#endif
#define ADB_PRODUCT_ID				0x0355//0x0002
#endif
#define ENABLE						1
#define DISABLE						0
...
//UmsCDEnable should be initialized from 0 to 1
volatile int UmsCDEnable = 1;//fxp 1->0 
...
int Android_switch_usb_conf (unsigned char new_usb_conf)
{
	...
	case MSC_CDROM_MODE:
		device_desc.idVendor = __constant_cpu_to_le16(VENDOR_ID);
		device_desc.idProduct = __constant_cpu_to_le16(CDROM_PRODUCT_ID);
		device_desc.bDeviceClass         = USB_CLASS_MASS_STORAGE;
		set_current_usb_config(SET_ANDROID_USB_CONF);
		//adb_interface_enable(DISABLE);		
		//dev->cdev->desc.bDeviceClass         = USB_CLASS_MASS_STORAGE;
		adb_interface_enable(DISABLE);
		dev->adb_enabled = DISABLE;//DISABLE;
		adb_function_enable(DISABLE);
        //dev->cdev->desc.bDeviceClass         = USB_CLASS_MASS_STORAGE;
		cur_adb_mode = 1;
		UmsCDEnable = 1;   
		if(debug_mask)
			printk("------cdrom---Android_switch_usb_conf---UmsCDEnable=%d---\n",UmsCDEnable);
		break;
#endif
	case ADB_MSC_MOD:
#ifdef SS_MODE
...
#if defined(CONFIG_ZTE_CDROM_SUPPORT)
	case MSC_CDROM_MODE:
		pr_info("------cdrom---adb_enable_ioctl->MSC cdrom mode---\n");
		enable_adb(_android_dev, MSC_CDROM_MODE);
		break;
#endif
	case ADB_MSC_MOD:

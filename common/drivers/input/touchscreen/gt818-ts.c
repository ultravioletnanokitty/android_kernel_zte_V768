...
//yuanlan@wind-mobi.com 2011.07.01 begin
...
/* yaogangxiang@wind-mobi.com 2012.5.28 begin */
//TP ESD
#define ESD_PROTECT
#if defined(ESD_PROTECT)
...
int tpd_halt = 0;
#endif
/* yaogangxiang@wind-mobi.com 2012.5.28 end */
...
//maxiaohui@wind-mobi.com 20120614 begin
void irq_status_disable(struct goodix_ts_data *ts)
{
...
void irq_status_enable(struct goodix_ts_data *ts)
{
     unsigned long irqflags;
     spin_lock_irqsave(&ts->lock, irqflags);
     if(!ts->irq_status)
...
//maxiaohui@wind-mobi.com 20120614 end
/*******************************************************
功能：
...
static int goodix_init_panel(struct goodix_ts_data *ts)
{
	...
	uint8_t config_info_d_lc[] = {		//lianchuang_Touch key devlop board
        ...
        0x06,0xA2,
       //yuanlan@wind-mobi.com 20120111 start
       //when tp has water,the tp will  no use
		...
		0x00,0x01
		 //yuanlan@wind-mobi.com 20120111 end
          } ;

	
	 //xiongbiao@wind-mobi.com 20120222 start
     //add tian ma tp config on L400 
	uint8_t config_info_tian_ma[] = 
	{		//tianma_Touch key devlop board
		0x06,0xA2,
		...
		0x71,0x11,0x07,0x03,0x10,0x10,0x10,0x39,
		0x39,0x39,0x10,0x0F,0x0A,0x40,0x25,0x0D,
		0x03,0x00,MAX_FINGER_NUM,(TOUCH_MAX_HEIGHT&0xff),(TOUCH_MAX_HEIGHT>>8),(TOUCH_MAX_WIDTH&0xff),(TOUCH_MAX_WIDTH>>8),0x00,
		0x00,0x5A,0x5C,0x5E,0x60,0x00,0x00,0x13,
		0x14,0x25,0x06,0x00,0x00,0x00,0x00,0x00,
		0x14,0x10,0xEF,0x03,0x00,0x00,0x00,0x00,
		0x00,0x00,0x1A,0x43,0x6C,0x98,0x0C,0x40,
		0x25,0x25,0x14,0x00,0x00,0x00,0x00,0x00,
		0x00,0x01

		};
//maxiaohui@wind-mobi.com begin 2012.04.10
#if defined(CONFIG_TOUCHSCREEN_GT818_STK)
	uint8_t config_info_santok[] =
	{
		0x06,0xA2,    
		0x00,0x02,0x04,0x06,0x08,0x0A,0x0C,0x0E,
		...
		0x40,0x00,0x30,0x00,0x20,0x00,0x00,0x00,
		0x10,0x00,0x07,0x03,0x10,0x10,0x10,0x32,
		0x32,0x32,0x0F,0x0E,0x0A,0x45,0x25,0x0D,
		0x03,0x00,MAX_FINGER_NUM,(TOUCH_MAX_HEIGHT&0xff),(TOUCH_MAX_HEIGHT>>8),(TOUCH_MAX_WIDTH&0xff),(TOUCH_MAX_WIDTH>>8),0x00,
		0x00,0x4C,0x51,0x50,0x54,0x00,0x00,0x25,
		0x14,0x25,0x05,0x00,0x00,0x00,0x00,0x00,
		0x14,0x10,0x53,0x03,0x00,0x00,0x00,0x00,
		0x00,0x00,0x18,0x43,0x70,0x96,0x0C,0x40,
		0x30,0x35,0x20,0x00,0x00,0x00,0x00,0x00,
		0x00,0x01
	};
#endif
//maxiaohui@wind-mobi.com end 2012.04.10
	 //xiongbiao@wind-mobi.com 20120222 end
	 
	  //xiongbiao@wind-mobi.com 20120227 start
	  //add xia lang tp config on L400 
#ifdef CONFIG_GT818_TP_XIA_LANG	  
	 uint8_t config_info_xia_lang[] = {		 //xialang_Touch key devlop board
		 0x06,0xA2,
		 0x00,0x02,0x04,0x06,0x08,0x0A,0x0C,0x0E,
		 ...
		 0xB1,0x11,0xC1,0x11,0xD1,0x11,0xE1,0x11,
		 0xF1,0x11,0x17,0x03,0x08,0x08,0x08,0x18,
		 0x18,0x18,0x10,0x0F,0x0A,0x45,0x2B,0x09,
		 0x03,0x00,MAX_FINGER_NUM,(480&0xff),(480>>8),(830&0xff),(830>>8),0x00,
		 0x00,0x58,0x54,0x5C,0x58,0x00,0x00,0x04,
		 0x20,0x05,0x05,0x00,0x00,0x00,0x00,0x00,
		 0x32,0x16,0xAC,0x03,0x00,0x00,0x00,0x00,
		 0x00,0x00,0x1A,0x41,0x6E,0x94,0x0C,0x40,
		 0x2B,0x20,0x14,0x00,0x00,0x00,0x00,0x00,
		 0x00,0x01
		 };
#endif
	  //xiongbiao@wind-mobi.com 20120227 end
...
//add for gt818,800 * 480
	...
	0x30,0x30,0x10,0x0F,0x0A,0x40,0x30,0x0D,//0x09
	0x03,0x00,MAX_FINGER_NUM,(TOUCH_MAX_HEIGHT&0xff),(TOUCH_MAX_HEIGHT>>8),(TOUCH_MAX_WIDTH&0xff),(TOUCH_MAX_WIDTH>>8),0x00,
...
/* yaogangxiang@wind-mobi.com 20120207 end */	
   ...
   else
   {
		...
		dev_info(&ts->client->dev,"Gutitar Version:D\n");
                ...
                if((ts->vendor) == 1 ){
                     ret=i2c_write_bytes(ts->client,config_info_d_jia, (sizeof(config_info_d_jia)/sizeof(config_info_d_jia[0])));		         
					 dev_info(&ts->client->dev,"Gutitar vendor=1 \n");
                }
			//xiongbiao@wind-mobi.com 20120222 start
    		 //add tian ma tp config on L400 
				else if((ts->vendor) == 0){
				...
				//add xia lang tp config on L400 
				#ifdef CONFIG_GT818_TP_XIA_LANG	
					ret=i2c_write_bytes(ts->client,config_info_xia_lang, (sizeof(config_info_xia_lang)/sizeof(config_info_xia_lang[0])));	
				#elif defined(CONFIG_TOUCHSCREEN_GT818_STK)
					ret=i2c_write_bytes(ts->client,config_info_santok, (sizeof(config_info_santok)/sizeof(config_info_santok[0])));					
                                #else
				//xiongbiao@wind-mobi.com 20120227 end
					ret=i2c_write_bytes(ts->client,config_info_tian_ma, (sizeof(config_info_tian_ma)/sizeof(config_info_tian_ma[0])));	
				#endif
					dev_info(&ts->client->dev,"Gutitar vendor=0\n");
				} 
			//xiongbiao@wind-mobi.com 20120222 end
				else if ((ts->vendor) == 2){
...
#endif
                      dev_info(&ts->client->dev,"Gutitar vendor=2\n");
              } else {
                          dev_info(&ts->client->dev,"nothing \n");
...
COORDINATE_POLL:
	...
	//			input_report_abs(ts->input_dev, ABS_MT_WIDTH_MAJOR, 0);	
			...
			{ 	
				//input_report_abs(ts->input_dev, ABS_MT_POSITION_Y, (*(coor_point+3*(position-1)+1))*SCREEN_MAX_WIDTH/(SCREEN_MAX_WIDTH)); //3.2 inch
			    //input_report_abs(ts->input_dev, ABS_MT_POSITION_X, (*(coor_point+3*(position-1)))*SCREEN_MAX_HEIGHT/(SCREEN_MAX_HEIGHT));

               input_report_abs(ts->input_dev, ABS_MT_POSITION_X, (*(coor_point+3*(position-1))));//2.8inch

				//xiongbiao@wind-mobi.com 20120308 start
				//fix problem that xia lang tp  is not accurate on L400 
			   #ifdef CONFIG_GT818_TP_XIA_LANG
			   ...
			   input_report_abs(ts->input_dev, ABS_MT_POSITION_Y,(*(coor_point+3*(position-1)+1))*SCREEN_MAX_WIDTH/(SCREEN_MAX_WIDTH));
				#endif
				//xiongbiao@wind-mobi.com 20120308 end
			   //input_report_abs(ts->input_dev, ABS_MT_POSITION_X, (*(coor_point+3*(position-1))));//2.8inch
		//	   input_report_abs(ts->input_dev, ABS_MT_POSITION_Y,(*(coor_point+3*(position-1)+1))*SCREEN_MAX_WIDTH/(SCREEN_MAX_WIDTH));
...
//	input_sync(ts->input_dev);	
	...
	last_key = key;	
#endif
        	input_sync(ts->input_dev);//key and touch all input_sync(ts->input_dev);	
	...
	switch(on)
	{
		...
		case 1:
			...
			if (ts->pdata->int_port)
			{
					...
					bcm_gpio_pull_up_down_enable(ts->pdata->int_port, false);
					if(ts->use_irq) 
					{
...
static u8  gt818_update_proc( u8 *nvram, u16 length, struct goodix_ts_data *ts )
{
    ...
    bcm_gpio_pull_up_down_enable(ts->pdata->int_port, false);//yuanlan
    //s3c_gpio_setpull(ts->pdata->int_port, S3C_GPIO_PULL_NONE);
...
static int goodix_ts_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	if(ts->pdata->tp_rst)
	{
    	bcm_gpio_pull_up_down_enable(ts->pdata->tp_rst, false);//yuanlan
...
/*  heweimao@wind-mobi.com 2012.09.11 end */
	...
	{
	 gpio_request(ts->pdata->tp_rst, "tp_reset");
	 printk("rina >>>>gpio_direction_output(ts->pdata->tp_rst,1)\n");
	gpio_direction_output(ts->pdata->tp_rst,1);
	msleep(20);
	gpio_direction_output(ts->pdata->tp_rst,0);
	msleep(20);
...
//maxiaohui@wind-mobi.com 20120614 begin
	...
	if (ts->pdata->int_port)
	{
		...
		gpio_direction_output(ts->pdata->int_port, 0);
		//bcm_gpio_pull_up_down_enable(ts->pdata->int_port, false);	//ret > 0 ?
		//(ts->pdata->int_port, INT_CFG);	//Set IO port function	
...
err_init_godix_ts:
	...
	if (ts->pdata->int_port)
		{
		        gpio_request(ts->pdata->int_port, "gt818-ts");
			gpio_direction_input(ts->pdata->int_port);
			gpio_free(ts->pdata->int_port);
		}	
	}
...
//TP ESD
#if defined(ESD_PROTECT)
static  int tpd_reset_func( struct goodix_ts_data *ts )
{
    int ret = 1;
    u8 retry;
...
static void tpd_esd_check_func(struct work_struct *work)
{
	...
	printk("tpd_esd_check_func begin \n");
	...
	if (tpd_halt)
	{
		printk("tpd_esd_check_func return .. \n");
		return;
	}
...
MODULE_LICENSE("GPL");
//yuanlan@wind-mobi.com 2011.07.01 end

...
int WriteBytes (unsigned int addr, unsigned int num)
{
   ...
   for(i=0;i<num;i++) SMB_DATA_OUT[i+9] = TYE_APP02_Binary[addr+i]; // temporary code data
   else if(TSP_MODULE_ID==0x03)
   for(i=0;i<num;i++) SMB_DATA_OUT[i+9] = SMAC_APP02_Binary[addr+i]; // temporary code data

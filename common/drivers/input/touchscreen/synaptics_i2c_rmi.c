...
#define PROXIMITY_1_FINGER_HOVER (1 << 0)
#define PROXIMITY_AIR_SWIPE (1 << 1)
#define PROXIMITY_LARGE_OBJ (1 << 2)
#define PROXIMITY_2_FINGER_HOVER ((1 << 4) | PROXIMITY_1_FINGER_HOVER)
#define PROXIMITY_ENABLE PROXIMITY_1_FINGER_HOVER
#endif
...
union synaptics_rmi4_f01_device_status {
	struct {
...
union synaptics_rmi4_f12_query_5 {
	struct {
...
union synaptics_rmi4_f12_query_8 {
	struct {
...
union synaptics_rmi4_f12_query_10 {
	struct {
...
union synaptics_rmi4_f12_ctrl_8 {
	struct {
...
union synaptics_rmi4_f12_ctrl_23 {
	struct {
...
union synaptics_rmi4_f12_finger_data {
	struct {
...
#if PROXIMITY
union synaptics_rmi4_f51_query {
	struct {
...
union synaptics_rmi4_f51_data {
	struct {
		unsigned char hover_finger1_x_pos_lsb;
		unsigned char hover_finger1_x_pos_msb;
		unsigned char hover_finger1_y_pos_lsb;
		unsigned char hover_finger1_y_pos_msb;
		unsigned char hover_finger1_z_distance;
		unsigned char hover_finger2_x_pos_lsb;
		unsigned char hover_finger2_x_pos_msb;
		unsigned char hover_finger2_y_pos_lsb;
		unsigned char hover_finger2_y_pos_msb;
		unsigned char hover_finger2_z_distance;
		unsigned char swipe_detection_direction;
		unsigned char x_swipe_velocity_lsb;
		unsigned char x_swipe_velocity_msb;
		unsigned char y_swipe_velocity_lsb;
		unsigned char y_swipe_velocity_msb;
		unsigned char large_obj_activity_detection;
	};
...
static bool exp_fn_inited = 0;
static struct mutex synaptics_rmi4_exp_fn_list_mutex;
static struct list_head synaptics_rmi4_exp_fn_list;


int virtual_key_event[][5]=
{
/*
{KEY_MENU,       158,1542,60,60},
{KEY_HOME,       385,1542,60,60},
{KEY_BACK,       620,1542,60,60},
{KEY_SEARCH,     850,1542,60,60}
*/
{KEY_MENU,		 158,1550,120,70},
{KEY_HOME,		 385,1550,120,70},
{KEY_BACK,		 620,1550,120,70},
{KEY_SEARCH,	         850,1550,120,70}
};
static int virtual_lastkey = 0;
...
{
	int retval;
	union synaptics_rmi4_f01_device_status device_status;
	struct synaptics_rmi4_data *rmi4_data = dev_get_drvdata(dev);
		...
		struct device_attribute *attr, const char *buf, size_t count)
{
	...
	if (rmi4_data->button_0d_enabled != input) {
		list_for_each_entry(function_handler,
			...
			if (function_handler->fn_number == SYNAPTICS_RMI4_0D_BUTTON_FN) {
				...
				if (input == 1)
					intr_enable |= function_handler->intr_mask;
				else
					intr_enable &= ~function_handler->intr_mask;
		...
		struct synaptics_rmi4_fn *function_handler)
{
	...
	for (finger = 0; finger < fingers_supported; finger++) {
		reg_index = finger / 4;
		finger_shift = (finger % 4) * 2; /* bit shift to get finger status */
		...
		if ((finger_status == 0x1) || (finger_status == 0x02)) {
			data_offset = data_base_addr + num_of_finger_status_regs +
			...
			if (retval < 0)
				...
				x[touch_count] = (data[0] << 4) | (data[2] & MASK_4BIT);
				y[touch_count] = (data[1] << 4) | ((data[2] >> 4) & MASK_4BIT);
				wx[touch_count] = (data[3] & MASK_4BIT);
				wy[touch_count] = (data[3] >> 4) & MASK_4BIT;

				if (rmi4_data->board->x_flip)
					x[touch_count] = rmi4_data->sensor_max_x - x[touch_count];
				if (rmi4_data->board->y_flip)
					y[touch_count] = rmi4_data->sensor_max_y - y[touch_count];
				...
				dev_dbg(&rmi4_data->i2c_client->dev, "%s Finger %d:\n"
						...
						"wx = %d\n"
						"wy = %d\n",
						__func__, finger, finger_status, x[touch_count],
						y[touch_count], wx[touch_count], wy[touch_count]);
			}
			touch_count++;
		}
	}
	...
	if (touch_count) {	
			for (finger = 0; finger < touch_count; finger++) {
				if(y[finger] > AXIS_RANGE_MAX_Y + 20 && y[finger] < virtual_key_event[0][2] + virtual_key_event[0][3]/2)
					...
					{
						if((x[finger] > virtual_key_event[retry][1]- virtual_key_event[retry][4])\
							&& (x[finger] < virtual_key_event[retry][1]+virtual_key_event[retry][4]))
						{
						   virtual_lastkey = virtual_key_event[retry][0];
						   input_report_key(rmi4_data->input_dev, virtual_key_event[retry][0],1);
						   key_flag = 1;
						} 
					}
				} else {
					input_report_abs(rmi4_data->input_dev, ABS_MT_POSITION_X,
							x[finger]);
					input_report_abs(rmi4_data->input_dev, ABS_MT_POSITION_Y,
							y[finger]);
		...
		struct synaptics_rmi4_fn *function_handler)
{
	...
	int wx;
	int wy;
	union synaptics_rmi4_f12_finger_data *data_base;
	union synaptics_rmi4_f12_finger_data *finger_data;

	fingers_supported = function_handler->num_of_data_points;
	data_base_addr = function_handler->full_addr.data_base;

	retval = synaptics_rmi4_i2c_read(rmi4_data,
			data_base_addr + function_handler->data1_offset,
			(unsigned char *)function_handler->data,
			function_handler->data_size);
	if (retval < 0)
		return 0;

	data_base = (union synaptics_rmi4_f12_finger_data *)function_handler->data;
		...
		struct synaptics_rmi4_fn *function_handler)
{
...
#if NO_0D_WHILE_2D
		memset(before_2d_button_status, 0, sizeof(before_2d_button_status));
		memset(while_2d_button_status, 0, sizeof(while_2d_button_status));
#endif
		do_once = 0;
	}
	...
	if (retval < 0) {
		dev_err(&rmi4_data->i2c_client->dev, "%s Failed to read button data "
				"registers\n", __func__);
		return;
	}
	...
	for (button = 0; button < f1a->valid_button_count; button++) {
		...
		button_status = ((data_base[button_reg] >> button_shift) & MASK_1BIT);
		...
		dev_dbg(&rmi4_data->i2c_client->dev, "%s Button %d (code %d) -> %d\n",
		...
		if (rmi4_data->fingers_on_2d == false) {
			...
			if (before_2d_button_status[button] == 1) {
				...
				input_report_key(rmi4_data->input_dev,
						f1a->button_map[button], button_status);
			} else {
				if (button_status == 1)
...
#if PROXIMITY
static int synaptics_rmi4_proximity_report(struct synaptics_rmi4_data *rmi4_data,
		struct synaptics_rmi4_fn *function_handler)
{
	int retval;
	unsigned char touch_count = 0; /* number of touch points */
	...
	int z;
	int swipe_velocity;
	union synaptics_rmi4_f51_data *data_register;

	data_base_addr = function_handler->full_addr.data_base;
	data_register = (union synaptics_rmi4_f51_data *)function_handler->data;

	retval = synaptics_rmi4_i2c_read(rmi4_data, data_base_addr,
			data_register->data, sizeof(data_register->data));
	if (retval < 0)
		return retval;

	if (PROXIMITY_ENABLE & PROXIMITY_1_FINGER_HOVER) {
		if (data_register->hover_finger1_z_distance > 0) {
			x = (data_register->hover_finger1_x_pos_msb << 8) |
					data_register->hover_finger1_x_pos_lsb;
			y = (data_register->hover_finger1_y_pos_msb << 8) |
					data_register->hover_finger1_y_pos_lsb;
			z = HOVER_Z_MAX - data_register->hover_finger1_z_distance;

			dev_dbg(&rmi4_data->i2c_client->dev, "%s Hover finger 1:\n"
					"x = %d\n"
					"y = %d\n"
					"z = %d\n",
					__func__, x, y, z);
			...
			input_report_abs(rmi4_data->input_dev, ABS_MT_POSITION_X, x);
			input_report_abs(rmi4_data->input_dev, ABS_MT_POSITION_Y, y);
			input_report_abs(rmi4_data->input_dev, ABS_MT_DISTANCE, z);
			input_mt_sync(rmi4_data->input_dev);

			touch_count++;
		}

		if ((PROXIMITY_ENABLE & PROXIMITY_2_FINGER_HOVER) &&
				(data_register->hover_finger2_z_distance > 0)) {
			x = (data_register->hover_finger2_x_pos_msb << 8) |
					data_register->hover_finger2_x_pos_lsb;
			y = (data_register->hover_finger2_y_pos_msb << 8) |
					data_register->hover_finger2_y_pos_lsb;
			z = HOVER_Z_MAX - data_register->hover_finger2_z_distance;

			dev_dbg(&rmi4_data->i2c_client->dev, "%s Hover finger 2:\n"
					"x = %d\n"
					"y = %d\n"
					"z = %d\n",
					__func__, x, y, z);
	...
	if (PROXIMITY_ENABLE & PROXIMITY_AIR_SWIPE) {
		if (data_register->swipe_detection_direction & 0x01) {
			swipe_velocity = (data_register->x_swipe_velocity_msb << 8) |
					data_register->x_swipe_velocity_lsb;
			dev_dbg(&rmi4_data->i2c_client->dev, "%s Swipe info = 0x%02x\n",
					__func__, data_register->swipe_detection_direction);
			dev_dbg(&rmi4_data->i2c_client->dev, "%s Swipe velocity x = %d\n",
					__func__, swipe_velocity);
		}

		if (data_register->swipe_detection_direction & 0x10) {
			swipe_velocity = (data_register->y_swipe_velocity_msb << 8) |
					data_register->y_swipe_velocity_lsb;
			dev_dbg(&rmi4_data->i2c_client->dev, "%s Swipe info = 0x%02x\n",
					__func__, data_register->swipe_detection_direction);
			dev_dbg(&rmi4_data->i2c_client->dev, "%s Swipe velocity y = %d\n",
					__func__, swipe_velocity);
		}
	}

	if (PROXIMITY_ENABLE & PROXIMITY_LARGE_OBJ) {
		if (data_register->large_obj_activity_detection & 0x04) {
			dev_dbg(&rmi4_data->i2c_client->dev, "%s Large object info = "
					"0x%02x\n", __func__,
					data_register->large_obj_activity_detection);
		}
	}
...
static void synaptics_rmi4_report_touch(struct synaptics_rmi4_data *rmi4_data,
		struct synaptics_rmi4_fn *function_handler, unsigned char *touch_count)
{
	...
	case SYNAPTICS_RMI4_PROXIMITY_FN:
		*touch_count += synaptics_rmi4_proximity_report(rmi4_data,
				function_handler);
		break;
#endif
	default:
...
//        printk("[test]:%s\n", __func__);
	...
	list_for_each_entry(function_handler, &rmi->support_fn_list, link) {
		if (function_handler->num_of_data_sources) {
			if (function_handler->intr_mask &
				...
				synaptics_rmi4_report_touch(rmi4_data, function_handler,
						&touch_count);
			}
		}
	}
	...
	if (!list_empty(&synaptics_rmi4_exp_fn_list)) {
		list_for_each_entry(exp_fn_handler, &synaptics_rmi4_exp_fn_list, link) {
			if (exp_fn_handler && (exp_fn_handler->func_attn != NULL)) {
				exp_fn_handler->func_attn(rmi4_data, intr_status[0]);
                        }
		}
	}
...
static irqreturn_t synaptics_rmi4_irq(int irq, void *data)
{
	...
	do {
		...
		if (touch_count > 0) {
//                        printk("[test]:touch_count > 0\n");
			wait_event_timeout(rmi4_data->wait, rmi4_data->touch_stopped,
					msecs_to_jiffies(SENSOR_POLLING_PERIOD));
		} else {
//                        printk("[test]:touch_count <= 0\n");
			break;
		}
		...
		bool enable)
{
	...
	if (enable) {
		...
		retval = request_threaded_irq(rmi4_data->irq, NULL, synaptics_rmi4_irq,
				platform_data->irq_type, DRIVER_NAME, rmi4_data);
		if (retval < 0) {
			dev_err(&rmi4_data->i2c_client->dev, "%s Failed to create irq "
					"thread\n", __func__);
			return retval;
		}
 ...
 *
 * This funtion parses information from the Function 11 registers and determines
 ...
 */
static int synaptics_rmi4_f11_2d_init(struct synaptics_rmi4_data *rmi4_data,
		struct synaptics_rmi4_fn *function_handler,
		struct synaptics_rmi4_fn_desc *fd, unsigned int intr_count)
{
	...
	retval = synaptics_rmi4_i2c_read(rmi4_data,
			function_handler->full_addr.query_base, query, sizeof(query));
	if (retval < 0)
		return retval;

	/* Maximum number of fingers supported */
	if ((query[1] & MASK_3BIT) <= 4)
		function_handler->num_of_data_points = (query[1] & MASK_3BIT) + 1;
	else {
	...
	retval = synaptics_rmi4_i2c_read(rmi4_data,
			function_handler->full_addr.ctrl_base, control, sizeof(control));
	if (retval < 0)
		return retval;
	...
	dev_dbg(&rmi4_data->i2c_client->dev, "%s Function %02x max x = %d max y = "
			"%d\n", __func__, function_handler->fn_number,
 ...
 * acquisition.
 */
static int synaptics_rmi4_f12_2d_init(struct synaptics_rmi4_data *rmi4_data,
		struct synaptics_rmi4_fn *function_handler,
		struct synaptics_rmi4_fn_desc *fd, unsigned int intr_count)
{
	int retval;
	...
	union synaptics_rmi4_f12_query_5 query_5;
	union synaptics_rmi4_f12_query_8 query_8;
	union synaptics_rmi4_f12_ctrl_8 ctrl_8;
	union synaptics_rmi4_f12_ctrl_23 ctrl_23;
	union synaptics_rmi4_f12_finger_data *finger_data_list;

	function_handler->fn_number = fd->fn_number;
	function_handler->num_of_data_sources = fd->intr_src_count;

	retval = synaptics_rmi4_i2c_read(rmi4_data,
			function_handler->full_addr.query_base + 5, query_5.data,
			sizeof(query_5.data));
	if (retval < 0)
		return retval;
	...
	retval = synaptics_rmi4_i2c_read(rmi4_data,
			function_handler->full_addr.ctrl_base + ctrl_23_offset,
	...
	retval = synaptics_rmi4_i2c_read(rmi4_data,
			function_handler->full_addr.query_base + 8,query_8.data,
			sizeof(query_8.data));
	if (retval < 0)
		return retval;
	...
	retval = synaptics_rmi4_i2c_read(rmi4_data,
			function_handler->full_addr.ctrl_base + ctrl_8_offset,
			ctrl_8.data, sizeof(ctrl_8.data));
	if (retval < 0)
		return retval;

	/* Maximum x and y */
	rmi4_data->sensor_max_x = ((unsigned short)ctrl_8.max_x_coord_lsb << 0) |
			((unsigned short)ctrl_8.max_x_coord_msb << 8);
	rmi4_data->sensor_max_y = ((unsigned short)ctrl_8.max_y_coord_lsb << 0) |
	...
	function_handler->data_size = function_handler->num_of_data_points *
			sizeof(union synaptics_rmi4_f12_finger_data);
...
static int synaptics_rmi4_0d_alloc_memory(struct synaptics_rmi4_data *rmi4_data,
		struct synaptics_rmi4_fn *function_handler)
{
	int retval;
	...
	if (!f1a) {
		dev_err(&rmi4_data->i2c_client->dev, "%s Failed to allocate memory "
				"for function handle\n", __func__);
		return -ENOMEM;
	}
	...
	retval = synaptics_rmi4_i2c_read(rmi4_data,
			function_handler->full_addr.query_base, f1a->button_query.data,
			sizeof(f1a->button_query.data));
	if (retval < 0) {
		dev_err(&rmi4_data->i2c_client->dev, "%s Failed to read query "
				"registers\n", __func__);
		return retval;
	}
...
static int synaptics_rmi4_0d_button_map(struct synaptics_rmi4_data *rmi4_data,
		struct synaptics_rmi4_fn *function_handler)
{
	unsigned char ii;
	struct synaptics_rmi4_f1a_handle *f1a = function_handler->data;
/*
	if (!rmi4_data->board->f1a_button_map) {
		dev_err(&rmi4_data->i2c_client->dev, "%s f1a_button_map is NULL in "
				"board file\n", __func__);
		return -ENODEV;
	} else if (!rmi4_data->board->f1a_button_map->map) {
		dev_err(&rmi4_data->i2c_client->dev, "%s Button map is missing in "
				"board file\n", __func__);
		return -ENODEV;
	} else {
		if (rmi4_data->board->f1a_button_map->nbuttons != f1a->button_count) {
		...
		for (ii = 0; ii < f1a->valid_button_count; ii++)
			f1a->button_map[ii] = rmi4_data->board->f1a_button_map->map[ii];
	}
*/
	return 0;
}
...
static int synaptics_rmi4_0d_button_init(struct synaptics_rmi4_data *rmi4_data,
		struct synaptics_rmi4_fn *function_handler,
		struct synaptics_rmi4_fn_desc *fd, unsigned int intr_count)
{
	int retval;
...
#if PROXIMITY
static int synaptics_rmi4_proximity_init(struct synaptics_rmi4_data *rmi4_data,
		struct synaptics_rmi4_fn *function_handler,
		struct synaptics_rmi4_fn_desc *fd, unsigned int intr_count)
{
	int retval;
	...
	union synaptics_rmi4_f51_query query_register;
	union synaptics_rmi4_f51_data *data_register;
	...
	retval = synaptics_rmi4_i2c_read(rmi4_data,
			function_handler->full_addr.query_base, query_register.data,
...
static int synaptics_rmi4_query_device(struct synaptics_rmi4_data *rmi4_data)
{
	...
	for (page_number = 0; page_number < PAGES_TO_SERVICE; page_number++) {
		for (pdt_entry_addr = PDT_START; pdt_entry_addr > PDT_END;
			...
			if (rmi_fd.fn_number) {
				dev_dbg(&rmi4_data->i2c_client->dev, "%s Function %02x found"
				...
				case SYNAPTICS_RMI4_DEVICE_CONTROL_FN:
					...
					retval = synaptics_rmi4_i2c_read(rmi4_data,
							rmi4_data->f01_data_base_addr, device_status.data,
							sizeof(device_status.data));
					if (retval < 0)
						return retval;

					if (device_status.flash_prog == 1) {
						pr_notice("%s Device in flash programming mode, "
								"status = 0x%02x\n", __func__,
				...
				case SYNAPTICS_RMI4_F11_2D_SENSOR_FN:
					if (rmi_fd.intr_src_count) {
						retval = synaptics_rmi4_alloc_fh(&function_handler,
								&rmi_fd, page_number);
						if (retval < 0) {
							dev_dbg(&rmi4_data->i2c_client->dev, "%s Failed "
						...
						retval = synaptics_rmi4_f11_2d_init(rmi4_data,
								function_handler, &rmi_fd, intr_count);
						if (retval < 0)
							return retval;
					}
					break;
				case SYNAPTICS_RMI4_F12_2D_SENSOR_FN:
					if (rmi_fd.intr_src_count) {
						...
						retval = synaptics_rmi4_f12_2d_init(rmi4_data,
								function_handler, &rmi_fd, intr_count);
						if (retval < 0)
							return retval;
					}
				...
				case SYNAPTICS_RMI4_0D_BUTTON_FN:
					if (rmi_fd.intr_src_count) {
						...
						retval = synaptics_rmi4_0d_button_init(rmi4_data,
								function_handler, &rmi_fd, intr_count);
						if (retval < 0)
							return retval;
					}
				...
				case SYNAPTICS_RMI4_PROXIMITY_FN:
					if (rmi_fd.intr_src_count) {
						...
						retval = synaptics_rmi4_proximity_init(rmi4_data,
								function_handler, &rmi_fd, intr_count);
						if (retval < 0)
							return retval;
					}
				...
				if (function_handler && rmi_fd.intr_src_count) {
					dev_dbg(&rmi4_data->i2c_client->dev, "%s Adding Function "
...
flash_prog_mode:
	...
	dev_dbg(&rmi4_data->i2c_client->dev, "%s Number of interrupt registers = "
			"%d\n", __func__, rmi4_data->number_of_interrupt_registers);

	retval = synaptics_rmi4_i2c_read(rmi4_data, rmi4_data->f01_query_base_addr,
	...
	rmi4_data->rmi4_mod_info.serial_number = ((f01_query[9] & MASK_7BIT) << 8) |
			(f01_query[10] & MASK_7BIT);
	memcpy(rmi4_data->rmi4_mod_info.product_id_string, &f01_query[11], 10);

	if (rmi4_data->rmi4_mod_info.manufacturer_id != 1) {
		dev_err(&rmi4_data->i2c_client->dev, "%s Non-Synaptics device found,"
				" manufacturer ID = %d\n", __func__,
	...
	if (data_sources) {
		...
		list_for_each_entry(function_handler, &rmi->support_fn_list, link) {
			if (function_handler->num_of_data_sources) {
				interrupt_mask[function_handler->index_to_intr_reg] |=
	...
	for (ii = 0; ii < MAX_INTR_REGISTERS; ii++) {
		if (interrupt_mask[ii] != 0x00) {
			dev_dbg(&rmi4_data->i2c_client->dev, "%s Interrupt enable mask %d "
					"= 0x%02x\n", __func__, ii, interrupt_mask[ii]);
			f01_ctrl_offset = rmi4_data->f01_ctrl_base_addr + 1 + ii;
			retval = synaptics_rmi4_i2c_write(rmi4_data, f01_ctrl_offset,
					&(interrupt_mask[ii]), sizeof(interrupt_mask[ii]));
			if (retval < 0)
				return retval;
		}
	}
...
static void synaptics_rmi4_detection_work(struct work_struct *work)
{
	...
	struct synaptics_rmi4_data *rmi4_data =
			container_of(work, struct synaptics_rmi4_data, detection_work.work);

	queue_delayed_work(rmi4_data->detection_workqueue,
			&rmi4_data->detection_work, msecs_to_jiffies(EXP_FN_DET_INTERVAL));
		...
		unsigned char intr_mask))
{
	...
	if (insert) {
		...
		if (!exp_fn_handler) {
			pr_err("%s Failed to allocate memory for expansion function\n",
					__func__);
			goto exit;
		}
		...
		exp_fn_handler->inserted = false;
		list_add_tail(&exp_fn_handler->link, &synaptics_rmi4_exp_fn_list);
	} else {
		list_for_each_entry(exp_fn_handler, &synaptics_rmi4_exp_fn_list, link) {
			if (exp_fn_handler && (exp_fn_handler->func_init == func_init)) {
...
/* yaogangxiang@wind-mobi.com 2012.0928 end */
	...
	if (!rmi4_data) {
		dev_err(&client->dev, "%s Failed to allocate memory for rmi4_data "
				"instance\n", __func__);
		return -ENOMEM;
	}
	...
	for (i=0; i < 5; i++)
	{
		__set_bit(virtual_key_event[i][0], rmi4_data->input_dev->keybit);
	}
...
#endif
        rmi4_data->sensor_max_y = AXIS_RANGE_MAX_Y;
	...
	if (!list_empty(&rmi4_data->rmi4_mod_info.support_fn_list)) {
		list_for_each_entry(function_handler,
				&rmi4_data->rmi4_mod_info.support_fn_list, link) {
			if (function_handler && (function_handler->fn_number ==
...
/* yaogangxiang@wind-mobi.com 2012.0928 end */
	...
	if (platform_data->gpio_config) {
		...
		if (retval < 0) {
			dev_err(&client->dev, "%s Failed to configure interrupt GPIO\n",
					__func__);
			goto err_enable_irq;
		}
	}
	...
	INIT_DELAYED_WORK(&rmi4_data->detection_work, synaptics_rmi4_detection_work);
	queue_delayed_work(rmi4_data->detection_workqueue,
			&rmi4_data->detection_work, msecs_to_jiffies(EXP_FN_DET_INTERVAL));
...
static void synaptics_rmi4_sensor_sleep(struct synaptics_rmi4_data *rmi4_data)
{
	...
	retval = synaptics_rmi4_i2c_write(rmi4_data, rmi4_data->f01_ctrl_base_addr,
			&device_ctrl, sizeof(device_ctrl));
	if (retval < 0) {
		dev_err(&(rmi4_data->input_dev->dev), "%s Failed to enter sleep mode\n",
				__func__);
...
static void synaptics_rmi4_sensor_wake(struct synaptics_rmi4_data *rmi4_data)
{
	...
	retval = synaptics_rmi4_i2c_read(rmi4_data, rmi4_data->f01_ctrl_base_addr,
			&device_ctrl, sizeof(device_ctrl));
	if (retval < 0) {
		dev_err(&(rmi4_data->input_dev->dev), "%s Failed to wake from sleep "
				"mode\n", __func__);
		rmi4_data->sensor_sleep = true;
		return;
	}

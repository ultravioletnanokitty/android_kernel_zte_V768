...
static int ilitek_i2c_reset_init(void);
static int ilitek_i2c_gpio_init(void);



#define DRIVER_VERSION "aimvB"
...
#define KEYPAD01_X1	20
#define KEYPAD01_X2	80
#define KEYPAD02_X1	95
#define KEYPAD02_X2	155
#define KEYPAD03_X1	170
#define KEYPAD03_X2	230
#define KEYPAD04_X1	245
#define KEYPAD04_X2	305
#define KEYPAD_Y	480
// definitions
...
#define DBG(fmt, args...)   if (1)printk("%s(%d): " fmt, __func__,__LINE__,  ## args)
	...
	int max_y)
{
	...
	for(key=0; key<sizeof(touch_key_code); key++){
        	...
        	//set_bit(touch_key_code[key] & KEY_MAX, input->keybit);//xiongbiao
        	input_set_capability(input,EV_KEY,touch_key_code[key] & KEY_MAX);	
	}
...
ilitek_i2c_process_and_report(
	void)
{
	int i, len, ret, x, y,key,org_x, org_y ;
	unsigned char key_id,key_flag=1;
	...
	if(i2c.protocol_ver & 0x200){
		...
		//input_sync(i2c.input_dev);//xiongbiao
	}
        return ret;
}
	...
	struct i2c_client *client, pm_message_t mesg)
{
	...
	if(i2c.valid_irq_request != 0){
		...
		ret = i2c_smbus_write_byte_data(i2c.client, 0x30, 0x00);
			if(ret < 0){
				printk(ILITEK_ERROR_LEVEL "%s, i2c write, failed\n", __func__);
			return -1;
		}		
...
{
        if(i2c.valid_irq_request != 0){
				ilitek_i2c_reset_init();
				//ilitek_i2c_gpio_init();
                enable_irq(i2c.client->irq);
...
static void acar_exit_gpio_exit(void)
{
	gpio_free(29);
}
	...
	void)
{
	...
	if(ret == 0){
		...
		if(ilitek_i2c_read_tp_info() < 0)
		{
			printk(ILITEK_ERROR_LEVEL "%s, read info error\n", __func__);
			return ret;
		}
		ilitek_i2c_gpio_init();

...
extern void synaptics_rmi4_new_function(enum exp_fn fn_type, bool insert,
...
static loff_t rmidev_llseek(struct file *filp, loff_t off, int whence)
{
	...
	if (newpos < 0 || newpos > REG_ADDR_LIMIT) {
		dev_err(&dev_data->rmi_dev->rmi4_data->i2c_client->dev, "%s New "
				"position 0x%04x is invalid\n", __func__, (unsigned int)newpos);
		newpos = -EINVAL;
		goto clean_up;
	}
...
{
	int retval = 0;
	struct rmidev_data *dev_data = container_of(inp->i_cdev, struct rmidev_data,
			main_dev);
...
static void rmidev_device_cleanup(struct rmidev_data *dev_data)
{
	...
	if (dev_data) {
		...
		dev_dbg(&dev_data->rmi_dev->rmi4_data->i2c_client->dev, "%s rmidev "
				"device removed\n", __func__);
	}
...
static int rmidev_init_device(struct synaptics_rmi4_data *rmi4_data)
{
	...
	if (retval < 0) {
		dev_err(&rmi4_data->i2c_client->dev, "%s Failed to create device "
				"class\n", __func__);
	...
	if (rmidev_major_num) {
		...
		if (retval < 0) {
			dev_err(&rmi4_data->i2c_client->dev, "%s Failed to allocate "
					"character device region for rmidev\n", __func__);
			goto err_device_region;
		}

		rmidev_major_num = MAJOR(dev_no);
		dev_dbg(&rmi4_data->i2c_client->dev, "%s Major number of rmidev = %d\n",
				__func__, rmidev_major_num);
	}

	dev_data = kzalloc(sizeof(*dev_data), GFP_KERNEL);
	if (!dev_data) {
		dev_err(&rmi4_data->i2c_client->dev, "%s Failed to allocate memory for "
				"rmidev_data\n", __func__);
		retval = -ENOMEM;
		goto err_dev_data;
	}
	...
	if (retval < 0) {
		dev_err(&rmi4_data->i2c_client->dev, "%s Failed to add rmi character "
				"device\n", __func__);
	...
	if (retval < 0) {
		dev_err(&rmi4_data->i2c_client->dev, "%s Failed to export attention "
				"gpio\n", __func__);
	} else {
		retval = gpio_export_link(&(rmi4_data->input_dev->dev), "attn",
				rmi4_data->board->gpio_irq);
		if (retval < 0) {
			dev_err(&rmi4_data->input_dev->dev, "%s Failed to create symlink "
					"for attention gpio\n", __func__);
		} else {
			dev_dbg(&rmi4_data->input_dev->dev, "%s Exported attention gpio "

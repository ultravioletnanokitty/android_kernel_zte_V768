...
//shaojiang@wind-mobi.com 2011.12.30 begin
//fix bug #6563.
//reviewed by liubing@wind-mobi.com
#include <linux/vmalloc.h>
//shaojiang@wind-mobi.com 2011.12.30 end
...
struct notifier_block *nb, unsigned long val, void *v)
{
	...
	if (!strncmp(v, "recovery", 8) || !strncmp(v, "fota-update", 11) || !strncmp(v, "fota-verify", 11))	{
		...
		#else
		flashblock = vmalloc(mtd->erasesize);
		#endif
		//shaojiang@wind-mobi.com 2011.12.30 end
		...
		while((addr < mtd->size) && (mtd->block_isbad(mtd,addr)))
			addr += mtd->erasesize;

		if (addr >= mtd->size) {
			pr_err("------recovery_problem---[Error]:%s,can not find a good block in misc partition\n", __func__);
			goto clean_up;
		}	
		/* read the BCB from the misc partition */
		/* read the entire block as we'll have to
		   rewrite it hence we need to erase */
		mtd->read(mtd, addr, mtd->erasesize, &retlen, flashblock);
        ...
        } else if (!strncmp(v, "fota-update", 11)) {
            /* set bcb.command to "boot-recovery" */
            strcpy(bcb->command, "fota-image");

            /* clean bcb.status */
            memset(bcb->status, 0, sizeof(bcb->status));

            /* set bcb.recovery to "recovery" */
            strcpy(bcb->recovery, "update");
        } else if (!strncmp(v, "fota-verify", 11)) {
            /* set bcb.command to "boot-recovery" */
            strcpy(bcb->command, "fota-image");
		...
		mtd->write(mtd, addr, mtd->erasesize, &retlen, flashblock);
	}

...
/*
	printk(KERN_INFO "AKM8962 %s: flag =0x%X", __func__,
		rbuf[0]);
...
/*	if (ready & ACC_DATA_READY) {
	...
	*/
	...
	if (ready & ORI_DATA_READY) {
        ...
        //heweimao@wind-mobi.com 2011.11.3 begin
        //compass  ABS_HAT1Y change
        //reviewed yuanlan@wind-mobi.com
		input_report_abs(akm->input, ABS_HAT1Y, rbuf[8]);
        //heweimao@wind-mobi.com 2011.11.3 end
	}
			...
			unsigned long arg)
{
	...
	short  ypr_buf[YPR_DATA_SIZE];	/* for SET_YPR */
...
static ssize_t akm8962_accel_show(
	struct device *dev, struct device_attribute *attr, char *buf)
{
	return 0;
}
...
static ssize_t akm8962_sysfs_accel_store(
	struct akm8962_data *akm, char const *buf, size_t count)
{
	int en = 0;
	...
	//printk(KERN_INFO "akm8962_sysfs_accel_store: buffer:%s   size=%d", buf, count);
...
static ssize_t akm8962_accel_store(
	struct device *dev, struct device_attribute *attr,
	char const *buf, size_t count)
{
	return akm8962_sysfs_accel_store(
...
//	printk(KERN_INFO "akm8962_bin_accel_write++: %x", akm);
	//printk(KERN_INFO "akm8962_bin_accel_write++: buffer:%s   size=%d", buf, size);
...
//	printk( KERN_INFO "akm8962_bin_accel_write Alok accel:%d,%d,%d\n",
//		accel_data[0], accel_data[1], accel_data[2]);
//	printk(KERN_INFO "akm8962_bin_accel_write--");
...
static irqreturn_t akm8962_irq(int irq, void *handle)
{
	...
	int err;
	int i;
	//printk(KERN_INFO "akm8962_irq: got interrupt\n");
	memset(buffer, 0, sizeof(buffer));
	buffer[0] = AK8962_REG_ST1;
	//liutao@wind-mobi.com 2012-3-29 begin
	//system halt,when starting live paper
	//yuanlan@wind-mobi.com 2012-3-29 review
	for(i=0;i<100;i++)
	{
		err = akm8962_i2c_rxdata(akm->i2c, buffer, 1);
		if (err < 0) {
	...
	//liutao@wind-mobi.com 2012-3-29 end
	mutex_lock(&akm->sensor_mutex);
...
{
	printk(KERN_INFO "akm8962_early_suspend\n");
	...
	AKECS_SetMode(s_akm, AK8962_MODE_POWERDOWN);
	wake_up(&s_akm->open_wq);
}

static void akm8962_late_resume(struct early_suspend *handler)
{
	printk(KERN_INFO "akm8962_late_resume\n");
...
int akm8962_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	dev_dbg(&client->dev, "akm8962_probe: start probing.");
...
#ifdef CONFIG_HAS_EARLYSUSPEND
		s_akm->akm_early_suspend.suspend = akm8962_early_suspend;
		s_akm->akm_early_suspend.resume = akm8962_late_resume;

...
//	msleep(10);
    ...
    al3006_i2c_write(0x04,0x5f); //0x4a is default value 0x5f is highest threshold
...
/* create a interface for test mode MobC00172048 */
            ...
            struct device_attribute *attr, char *buf)
{
    ...
    ret = al3006_i2c_read(0x05,&value);

    ret = sprintf(buf, "ADC0x05[0x%03X]\n testmode[%d]\n ", \
              value, lpi->testmode);
...
static DEVICE_ATTR(test_mode, 0664, test_mode_show, test_mode_store);
/**************sys interface end ****************/
	...
	const struct i2c_device_id *id)
{
    ...
    /* register the attributes */
    ret = device_create_file(lpi->ps_dev, &dev_attr_test_mode);
    if (ret)
        goto err_create_ps_device_file3;
...
err_create_ps_device_file3:
    device_remove_file(lpi->ps_dev, &dev_attr_test_mode);

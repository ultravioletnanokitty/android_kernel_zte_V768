...
int max89xx_audio_set_mixer_input(int audio_path, max8986_mixer_input input)
{
...
EXPORT_SYMBOL(max89xx_audio_set_mixer_input);

int max89xx_set_input_mode(int input_path, int mode)
{
	struct max8986 *max8986 = driv_data->max8986;
	u8 regVal;
	int ret;
...
EXPORT_SYMBOL(max89xx_set_input_mode);

int max89xx_set_input_preamp_gain(int input_path, int gain)
{
	struct max8986 *max8986 = driv_data->max8986;
	u8 regVal;
	int ret;
...
EXPORT_SYMBOL(max89xx_set_input_preamp_gain);

int max89xx_set_hs_preamp_gain(int gain)
{
	struct max8986 *max8986 = driv_data->max8986;
...
EXPORT_SYMBOL(max89xx_set_hs_preamp_gain);

int max89xx_set_ihf_preamp_gain(int gain)
{
	struct max8986 *max8986 = driv_data->max8986;
	struct max8986_audio_pdata *audio_pdata = max8986->pdata->audio_pdata;
	u8 regVal;
...
EXPORT_SYMBOL(max89xx_set_ihf_preamp_gain);

int max89xx_audio_hs_poweron(bool power_on)
{
	struct max8986 *max8986 = driv_data->max8986;
	u8 regVal;
	int ret;
...
EXPORT_SYMBOL(max89xx_audio_hs_poweron);

int max89xx_audio_hs_shortcircuit_enable(bool enable)
{
	/*struct max8986 *max8986 = driv_data->max8986; */
	pr_info("Inside %s\n", __func__);
...
EXPORT_SYMBOL(max89xx_audio_hs_shortcircuit_enable);

int max89xx_audio_hs_set_gain(int hspath, int hsgain)
{
	struct max8986 *max8986 = driv_data->max8986;
	u8 regVal;
	int ret = 0;
...
EXPORT_SYMBOL(max89xx_audio_hs_set_gain);

int max89xx_audio_hs_ihf_poweron(void)
{
	struct max8986 *max8986 = driv_data->max8986;
	u8 regVal;
	int ret;
...
EXPORT_SYMBOL(max89xx_audio_hs_ihf_poweron);

int max89xx_audio_hs_ihf_poweroff(void)
{
	struct max8986 *max8986 = driv_data->max8986;
	u8 regVal;
	int ret;
...
EXPORT_SYMBOL(max89xx_audio_hs_ihf_poweroff);

int max89xx_audio_hs_ihf_enable_bypass(bool enable)
{
	struct max8986 *max8986 = driv_data->max8986;
	u8 regVal;
	int ret;
...
EXPORT_SYMBOL(max89xx_audio_hs_ihf_enable_bypass);

int max89xx_audio_hs_ihf_set_gain(int ihfgain)
{
	struct max8986 *max8986 = driv_data->max8986;
	u8 regVal;
	int ret;
...
EXPORT_SYMBOL(max89xx_audio_hs_ihf_set_gain);

int max89xx_audio_enable_vol_slew(int enable)
{
	struct max8986 *max8986 = driv_data->max8986;
	u8 reg_val;
	int ret;
...
EXPORT_SYMBOL(max89xx_audio_enable_vol_slew);

int max89xx_audio_enable_zcd(int enable)
{
	struct max8986 *max8986 = driv_data->max8986;
	u8 reg_val;
	int ret;
...
EXPORT_SYMBOL(max89xx_audio_enable_zcd);

static int __devinit max89xx_audio_probe(struct platform_device *pdev)
{
	u8 reg_val;
	...
	if (max8986->pdata && max8986->pdata->audio_pdata) {
		...
		max89xx_audio_set_mixer_input(MAX8986_AUDIO_HS_RIGHT,
					      audio_pdata->rhs_def_mixer_in);
		max89xx_audio_set_mixer_input(MAX8986_AUDIO_HS_LEFT,
					      audio_pdata->lhs_def_mixer_in);
		max89xx_audio_set_mixer_input(MAX8986_AUDIO_SPK,
					      audio_pdata->ihf_def_mixer_in);

		max89xx_set_input_preamp_gain(MAX8986_INPUTA,
					      audio_pdata->ina_def_preampgain);
		max89xx_set_input_preamp_gain(MAX8986_INPUTB,
					      audio_pdata->inb_def_preampgain);

		max89xx_set_input_mode(MAX8986_INPUTA,
				       audio_pdata->ina_def_mode);
		max89xx_set_input_mode(MAX8986_INPUTB,
...
static int __devexit max89xx_audio_remove(struct platform_device *pdev)
{
...
static struct platform_driver max8986_audio_driver = {
	...
	.remove = __devexit_p(max89xx_audio_remove),
	.probe = max89xx_audio_probe,
};

static int __init max89xx_audio_init(void)
{
	return platform_driver_register(&max8986_audio_driver);
}

late_initcall(max89xx_audio_init);

static void __exit max89xx_audio_exit(void)
{
	platform_driver_unregister(&max8986_audio_driver);
}

module_exit(max89xx_audio_exit);

MODULE_LICENSE("GPL");
MODULE_DESCRIPTION("Audio Interface Driver for MAX89xx PMU");
MODULE_ALIAS("platform:max89xx-audio");

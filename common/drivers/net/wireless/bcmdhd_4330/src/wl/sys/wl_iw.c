...
#if 0	//penguin, there is no WPA_AUTH_WAPI here...
...
wl_iw_event(struct net_device *dev, wl_event_msg_t *e, void* data)
{
	...
	case WLC_E_REASSOC_IND:
		...
		if (ap_cfg_running) {
			char *macaddr = (char *)&e->addr;
			WL_SOFTAP(("ap_cfg_running\n"));
			wl_iw_send_priv_event(priv_dev, "STA_JOIN");
			add_uevent_var(&env, "SOFTAP=STA_JOIN,%02x:%02x:%02x:%02x:%02x:%02x",
	...
	case WLC_E_DEAUTH:
		...
		if (ap_cfg_running) {
			...
			add_uevent_var(&env, "SOFTAP=STA_LEAVE,%02x:%02x:%02x:%02x:%02x:%02x",
				macaddr[0], macaddr[1], macaddr[2], macaddr[3], macaddr[4], macaddr[5]);
			kobject_uevent_env(&(priv_dev->dev.kobj), KOBJ_CHANGE, (char**)&env);
			goto wl_iw_event_end;
		}

...
static const int csr_dvs_vol_max8986[][2] = {
...
static const int iosr_vol_max8986[][2] = {
...
static struct max8986_regl max8986_regls[] = {
	...
	MAX8986_DEFINE_REGL(CSR_NM1, MAX8986_PM_REG_CSROPMODCTRL,
			MAX8986_PM_REG_CSRCTRL10, csr_dvs_vol_max8986),
	MAX8986_DEFINE_REGL(CSR_NM2, MAX8986_PM_REG_CSROPMODCTRL,
			MAX8986_PM_REG_CSRCTRL3, csr_dvs_vol_max8986),
	MAX8986_DEFINE_REGL(CSR_LPM, MAX8986_PM_REG_CSROPMODCTRL,
			MAX8986_PM_REG_CSRCTRL1, csr_dvs_vol_max8986),

	MAX8986_DEFINE_REGL(IOSR, MAX8986_PM_REG_IOSROPMODCTRL,
			MAX8986_PM_REG_IOSRCTRL2, iosr_vol_max8986),
};

int max89xx_csr_reg2volt(int reg)
{
	int volt = -EINVAL;
	int i;
	const int (*vlist)[2] = max8986_regls[MAX8986_REGL_CSR_NM1].vol_list;
	int num = max8986_regls[MAX8986_REGL_CSR_NM1].num_vol;
...
EXPORT_SYMBOL(max89xx_csr_reg2volt);

static u8 max89xx_regulator_ctrl_reg_mask(int regl_id, int *bitPos)
{
	u8 mask = 0;
	switch (regl_id) {
...
static int _max89xx_regulator_enable(struct max8986_regl_priv *regl_priv,
									int id)
{
...
static int max89xx_regulator_enable(struct regulator_dev *rdev)
{
	struct max8986_regl_priv *regl_priv = rdev_get_drvdata(rdev);
	int id = rdev_get_id(rdev);
	return _max89xx_regulator_enable(regl_priv, id);
}

static int _max89xx_regulator_disable(struct max8986_regl_priv *regl_priv,
									int id)
{
	struct max8986 *max8986 = regl_priv->max8986;
	int ret;
...
static int max89xx_regulator_disable(struct regulator_dev *rdev)
{
	struct max8986_regl_priv *regl_priv = rdev_get_drvdata(rdev);
	int id = rdev_get_id(rdev);

	return _max89xx_regulator_disable(regl_priv, id);
}

static int _max89xx_regulator_is_enabled(struct max8986_regl_priv *regl_priv,
									int id)
{
	struct max8986 *max8986 = regl_priv->max8986;
	int ret;
...
static int max89xx_regulator_is_enabled(struct regulator_dev *rdev)
{
	struct max8986_regl_priv *regl_priv = rdev_get_drvdata(rdev);
	int id = rdev_get_id(rdev);
	return _max89xx_regulator_is_enabled(regl_priv, id);
}

static int max89xx_get_best_voltage_inx(int reg_id, int min_uV, int max_uV)
{
...
static int _max89xx_regulator_set_voltage(struct max8986_regl_priv *pri_dev,
							int id, int volt)
{
	...
	if ((id < 0) || (id >= MAX8986_REGL_NUM_REGULATOR))
		return -EINVAL;
	mask = max89xx_regulator_ctrl_reg_mask(id, &bitPos);
	if (!mask)
		return -EINVAL;
...
static int max89xx_regulator_set_voltage(struct regulator_dev *rdev,
						int min_uV, int max_uV)
{
	...
	/* Find the best index */
	inx = max89xx_get_best_voltage_inx(id, min_uV, max_uV);

	if (inx < 0 || inx >= max8986_regls[id].num_vol)
		return inx;
	value = max8986_regls[id].vol_list[inx][1];

	return _max89xx_regulator_set_voltage(regl_priv, id, value);
}

static int _max89xx_regulator_get_voltage(struct max8986_regl_priv *pri_dev,
									int id)
{
	int ret, val;
	u8 mask, regVal;
...
static int max89xx_regulator_get_voltage(struct regulator_dev *rdev)
{
	struct max8986_regl_priv *regl_priv = rdev_get_drvdata(rdev);
	int id = rdev_get_id(rdev);

	return _max89xx_regulator_get_voltage(regl_priv, id);
}

static int _max89xx_regulator_set_mode(struct max8986_regl_priv *pri_dev,
...
static int max89xx_regulator_set_mode(struct regulator_dev *rdev,
							unsigned int mode)
{
	u8 opmode;
	struct max8986_regl_priv *regl_priv = rdev_get_drvdata(rdev);
	...
	return _max89xx_regulator_set_mode(regl_priv, id, opmode);

}

static int _max89xx_regulator_get_mode(struct max8986_regl_priv *pri_dev,
									int id)
{
	struct max8986_regl_priv *regl_priv = pri_dev;
	struct max8986 *max8986 = regl_priv->max8986;
...
static unsigned int max89xx_regulator_get_mode(struct regulator_dev *rdev)
{
	u8 mode;
	struct max8986_regl_priv *regl_priv = rdev_get_drvdata(rdev);
	int id = rdev_get_id(rdev);

	mode = _max89xx_regulator_get_mode(regl_priv, id);
	pr_info("%s:opmode = %d\n", __func__, mode);
	switch(mode)
	{
	case PMU_REGL_ON:
...
static struct regulator_ops max8986_regulator_ops = {
	.enable = max89xx_regulator_enable,
	.disable = max89xx_regulator_disable,
	.is_enabled = max89xx_regulator_is_enabled,
	.set_voltage = max89xx_regulator_set_voltage,
	.get_voltage = max89xx_regulator_get_voltage,
	.set_mode = max89xx_regulator_set_mode,
	.get_mode = max89xx_regulator_get_mode,
};
...
static int max8986_regulator_ioctl_handler(u32 cmd, u32 arg, void *pri_data)
{
	...
	case BCM_PMU_IOCTL_SET_VOLTAGE:
	{
		...
		inx = max89xx_get_best_voltage_inx(
				regulator.regl_id,
				regulator.voltage,
				regulator.voltage);
		...
		ret = _max89xx_regulator_set_voltage(regl_priv,
				regulator.regl_id,
				value);
		break;
	}
	case BCM_PMU_IOCTL_GET_VOLTAGE:
	{
		...
		volt  = _max89xx_regulator_get_voltage(
				regl_priv,
				regulator.regl_id);
		if (volt > 0) {
	...
	case BCM_PMU_IOCTL_GET_REGULATOR_STATE:
	{
		...
		rmode.state = _max89xx_regulator_get_mode(
				regl_priv,
				rmode.regl_id);
		if (rmode.state > 0)
	...
	case BCM_PMU_IOCTL_SET_REGULATOR_STATE:
	{
		...
		if (copy_from_user(&rmode, (pmu_regl *)arg,
					sizeof(pmu_regl)) != 0)
			return -EFAULT;
		ret = _max89xx_regulator_set_mode(regl_priv,
				rmode.regl_id,
				rmode.state);
		break;
	}
...
static int __devinit max89xx_regulator_probe(struct platform_device *pdev)
{
	struct max8986_regl_priv *regl_priv;
	...
	/* CSR and IOSR voltage tables change with MAX PMU revisions.
	 * Update the tables here depending on the revision.
	 */
	if (max8986->rev == MAXIM_PMU_MAX8999_R0) {
		const int (*vlist)[2];
		int num;

		pr_info("Detected MAX8999 PMIC\n");

		vlist = csr_dvs_vol_max8999r0;
		num = ARRAY_SIZE(csr_dvs_vol_max8999r0);

		max8986_regls[MAX8986_REGL_CSR_NM1].vol_list = vlist;
		max8986_regls[MAX8986_REGL_CSR_NM1].num_vol = num;

		max8986_regls[MAX8986_REGL_CSR_NM2].vol_list = vlist;
		max8986_regls[MAX8986_REGL_CSR_NM2].num_vol = num;

		max8986_regls[MAX8986_REGL_CSR_LPM].vol_list = vlist;
		max8986_regls[MAX8986_REGL_CSR_LPM].num_vol = num;

		vlist = iosr_vol_max8999r0;
		num = ARRAY_SIZE(iosr_vol_max8999r0);

		max8986_regls[MAX8986_REGL_IOSR].vol_list = vlist;
		max8986_regls[MAX8986_REGL_IOSR].num_vol = num;
	}
	...
	max89xx_register_ioctl_handler(max8986, MAX8986_SUBDEV_REGULATOR,
...
static int __devexit max89xx_regulator_remove(struct platform_device *pdev)
{
...
static struct platform_driver max8986_regulator_driver = {
	...
	.remove		= __devexit_p(max89xx_regulator_remove),
	.probe		= max89xx_regulator_probe,
};

static int __init max89xx_regulator_init(void)
{
	return platform_driver_register(&max8986_regulator_driver);
}
subsys_initcall(max89xx_regulator_init);

static void __exit max89xx_regulator_exit(void)
{
	platform_driver_unregister(&max8986_regulator_driver);
}
module_exit(max89xx_regulator_exit);

MODULE_LICENSE("GPL");
MODULE_DESCRIPTION("Regulator Driver for MAX89xx PMU");
MODULE_ALIAS("platform:max89xx-regulator");

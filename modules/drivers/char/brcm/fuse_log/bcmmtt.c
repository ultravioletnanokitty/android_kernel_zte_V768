...
static const int MTTLOG_NumFrameBytes = MTT_HEADER_SIZE + MTT_PAYLOAD_CS_SIZE;	///<	number of frame (overhead) bytes 
...
{
	if (!is_log_err_once)
		log_error.logLostStartTime = timer_get_tick_count() / 32 ;
...
unsigned int BCMLOG_BuildLogLossMessage(char *ptr)
{
	...
	slen = MTTLOG_StringLen(ptr + MTT_HEADER_SIZE);

...
Boolean SYSPARM_GetImeiStr(UInt8 sim_id, UInt8* inImeiStr)
{
 ...
 Int16 SYSPARM_GetBattTemp_by_ADC(UInt8 ADC_index)
  {
  	UInt32 default_temp_table_addr;
	Int16* default_temp_table_ptr;
	Int16 batt_temp;
	if(!fuse_sysparm_initialised)
	{
		if(sysparm_init())
	...
	default_temp_table_addr = SYSPARM_GePAtByIndex("adc_temp2",256*sizeof(*default_temp_table_ptr),1);
	if(!default_temp_table_addr)
	{
		pr_err("[sysparm]:Get adc_temp2 PA failed\n");
		return 0;
	}
	default_temp_table_ptr = (UInt16*)ioremap_nocache(default_temp_table_addr,256*sizeof(*default_temp_table_ptr));
	if(!default_temp_table_ptr)
	{
		pr_err("[sysparm]:default_temp_table_addr ioremap failed\n");
		return 0;
	}
	batt_temp = default_temp_table_ptr[ADC_index];
	iounmap(default_temp_table_ptr);
	return batt_temp;
}

...
static volatile UInt8 CMAS_FEATURE_FLAG[DUAL_SIM_SIZE] = {CMAS_DISABLE,CMAS_DISABLE,CMAS_DISABLE};
...
Boolean g_bStopCBServiceAtFlightMode = FALSE;
#endif
...
void KRIL_SetBroadcastSmsHandler(void *ril_cmd, Kril_CAPI2Info_t *capi2_rsp)
{
    ...
    if (capi2_rsp != NULL)
        KRIL_DEBUG(DBG_ERROR, "handler_state:0x%lX::result:%d SIMID:%d CMAS_FLAG:%d\n", pdata->handler_state, capi2_rsp->result, pdata->ril_cmd->SimId, CMAS_FEATURE_FLAG[pdata->ril_cmd->SimId]);

//luobiao@wind-mobi.com start 2012.05.17
//patch for csp:524605
#ifdef SEND_STOP_CB_SERVICE_IN_FLIGHT_MODE
    if(g_bStopCBServiceAtFlightMode == FALSE) {        
        g_bStopCBServiceAtFlightMode = TRUE;
        KRIL_DEBUG(DBG_ERROR, "SIMID:%d. change g_bStopCBServiceAtFlightMode to %d in the first time of Set CB.\n",pdata->ril_cmd->SimId,g_bStopCBServiceAtFlightMode);
    }  
#else
    KRIL_DEBUG(DBG_ERROR, "SIMID:%d. STOP_CB_SERVICE_ENABLE does not define.\n",pdata->ril_cmd->SimId);
#endif
//luobiao@wind-mobi.com end 2012.05.17
    switch (pdata->handler_state)
    {
        case BCM_SendCAPI2Cmd:
        {
            ...
            if(Msg_Lang_mode == 1) {
                ...
                if((tdata->fromServiceId == 4352) && (tdata->toServiceId == 6399)) {
                    if(mode == 1) {
                        KRIL_DEBUG(DBG_ERROR, "SIMID:%d Start CMAS Feature!\n",pdata->ril_cmd->SimId);
                        CMAS_FEATURE_FLAG[pdata->ril_cmd->SimId] = CMAS_ENABLE;
                    ...
                    if(mode ==0) {
                        KRIL_DEBUG(DBG_ERROR, "SIMID:%d Stop CMAS Feature!\n",pdata->ril_cmd->SimId);
                        CMAS_FEATURE_FLAG[pdata->ril_cmd->SimId] = CMAS_DISABLE;
                    ...
                    if(mode == 1) {
                        ...
                        if(MsgId_Ptr != NULL) {
                            for(i=0;i<MsgId_Number;i++) {
                                MsgId_Ptr[2*i] = (0xff & tdata->fromServiceId);
                                MsgId_Ptr[2*i+1] = (0xff & tdata->fromServiceId>>8);
                                KRIL_DEBUG(DBG_INFO, "MsgId_Ptr[%d]=%x, MsgId_Ptr[%d]=%x\n", 2*i,MsgId_Ptr[2*i],2*i+1,MsgId_Ptr[2*i+1]);
                            }
                            CAPI2_SmsApi_AddCellBroadcastChnlReq(InitClientInfo(pdata->ril_cmd->SimId), MsgId_Ptr, MsgId_Number*2);
                            if (CMAS_FEATURE_FLAG[pdata->ril_cmd->SimId] == CMAS_ENABLE){
                                KRIL_DEBUG(DBG_ERROR, "SIMID:%d CMAS_ENABLE\n",pdata->ril_cmd->SimId);
                                pdata->handler_state = BCM_SMS_StartCBCMASService;
                            }
                            else{
                                KRIL_DEBUG(DBG_ERROR, "SIMID:%d CMAS_DISABLE\n",pdata->ril_cmd->SimId);
        ...
        case BCM_SMS_StartCBService:
        {
            ...
            KRIL_DEBUG(DBG_ERROR, "Add or Delete MsgID or LangID OK, CMAS DISABLE \n");
            CAPI2_SmsApi_StartReceivingCellBroadcastReq(InitClientInfo(pdata->ril_cmd->SimId));
            pdata->handler_state = BCM_RESPCAPI2Cmd;
            break;
        }
        ...
        case BCM_SMS_StartCBCMASService:
        {
            ...
            KRIL_DEBUG(DBG_ERROR, "Add or Delete MsgID or LangID OK, CMAS ENABLE \n");
            CAPI2_SmsApi_StartCellBroadcastForCMASReq(InitClientInfo(pdata->ril_cmd->SimId), &cbmi, NULL);
            pdata->handler_state = BCM_RESPCAPI2Cmd;
            break;
        }
        case BCM_RESPCAPI2Cmd:
        {
            ...
            {
                if (CMAS_FEATURE_FLAG[pdata->ril_cmd->SimId] == CMAS_ENABLE){
                    KRIL_DEBUG(DBG_ERROR, "SIMID:%d Start CB CMAS Service Fail, error result:%d \n",pdata->ril_cmd->SimId,capi2_rsp->result);
                }
                else{
                    KRIL_DEBUG(DBG_ERROR, "SIMID:%d Start CB Normal Service Fail, error result:%d \n",pdata->ril_cmd->SimId,capi2_rsp->result);
                }
                pdata->handler_state = BCM_ErrorCAPI2Cmd;
                return;
            }
            ...
            if (CMAS_FEATURE_FLAG[pdata->ril_cmd->SimId] == CMAS_ENABLE){
                KRIL_DEBUG(DBG_ERROR, "SIMID:%d Start CB CMAS Service OK \n",pdata->ril_cmd->SimId);
            }
            else{
                KRIL_DEBUG(DBG_ERROR, "SIMID:%d Start CB Normal Service OK \n",pdata->ril_cmd->SimId);
            }
...
void KRIL_SmsBroadcastActivationHandler(void *ril_cmd, Kril_CAPI2Info_t *capi2_rsp)
{
    ...
    switch (pdata->handler_state)
    {
        case BCM_SendCAPI2Cmd:
        {
            ...
            {                
                if(CMAS_FEATURE_FLAG[pdata->ril_cmd->SimId] == CMAS_ENABLE){
                    KRIL_DEBUG(DBG_INFO, "SIMID:%d Start CB CMAS Service\n",pdata->ril_cmd->SimId);                    
                    ...
                    KRIL_DEBUG(DBG_INFO, "SIMID:%d Start CB Normal Service\n",pdata->ril_cmd->SimId);
                    CAPI2_SmsApi_StartReceivingCellBroadcastReq(InitClientInfo(pdata->ril_cmd->SimId));                
                }
            }
            else //turn-off

...
static const struct xdr_discrim CAPI2_MSElement_t_dscrm[] = {
		...
		{ (int)MS_LOCAL_PBK_ELEM_UNICODE_OPTIMIZE_CTR,_T("MS_LOCAL_PBK_ELEM_UNICODE_OPTIMIZE_CTR"), (xdrproc_t)xdr_u_char, sizeof( UInt8 ), NULL_capi2_proc_t, (xdrproc_t)xdr_u_char ,0 },
...
bool_t xdr_MS_UmtsAmrInfo(void* xdrs,MS_UmtsAmrInfo  *param)
{
	...
	if(	
		_xdr_u_char(xdrs, &param->umts_amr_codec_mode, "umts_amr_codec_mode") &&
...
bool_t xdr_MS_UMTSParam_t(void* xdrs, MS_UMTSParam_t *param)
{
	...
	if(	_xdr_u_char(xdrs, &param->valid,"valid")  &&
        ...
        _xdr_u_int16_t(xdrs, &param->Srxlev,"Srxlev") &&
        _xdr_u_int16_t(xdrs, &param->Squal,"Squal") &&
        _xdr_u_char(xdrs, &param->tgmp_status,"tgmp_status") 
		)
	    return(TRUE);
	else
		return(FALSE);

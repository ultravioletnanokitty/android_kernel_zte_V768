...
bool_t
xdr_PCHDSMIPAddr_t(XDR *xdrs, PCHDSMIPAddr_t *args)
{
	XDR_LOG(xdrs,"PCHDSMIPAddr_t ")

	if( xdr_opaque(xdrs, (caddr_t)args, sizeof(PCHDSMIPAddr_t)))
		return(TRUE);
	else
		return(FALSE);
...
xdr_PCHDecodedProtConfig_t(XDR *xdrs, PCHDecodedProtConfig_t *args)
{
	...
	if(	xdr_PCHPCsCfAddr_t(xdrs, &args->pcscfAddr) && 
		...
		xdr_PCHDNSAddr_t(xdrs, &args->v6dnsPri) &&
		xdr_PCHDNSAddr_t(xdrs, &args->v6dnsSec) &&
		xdr_PCHDSMIPAddr_t(xdrs, &args->v6haAddr) &&
		xdr_PCHDSMIPAddr_t(xdrs, &args->v6hnPrefix) &&
		xdr_PCHDSMIPAddr_t(xdrs, &args->v6v4haAddr) &&

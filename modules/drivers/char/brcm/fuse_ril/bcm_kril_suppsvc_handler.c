...
void KRIL_SetCLIRHandler(void *ril_cmd, Kril_CAPI2Info_t *capi2_rsp)
{
    ...
    switch(pdata->handler_state)
    {
	    ...
	    // liutao@wind-mobi.com 2012-1-10 begin
	    // setting always need waiting
	    // liubin@wind-mobi.com 2012-1-10 review
           // KRIL_SetSsSrvReqTID(GetTID()); // MSG_MNSS_CLIENT_SS_SRV_REL is an asynchronous response. TID=0
            CAPI2_SsApi_SsSrvReq(InitClientInfo(pdata->ril_cmd->SimId), &ssApiSrvReq);

            KRIL_SetSsSrvReqTID(GetTID()); // MSG_MNSS_CLIENT_SS_SRV_REL is an asynchronous response. TID=0
	    //liutao@wind-mobi.com 2012-1-10 end
            pdata->handler_state = BCM_RESPCAPI2Cmd;
        }
        break;
            ...
            if (capi2_rsp->msgType == MSG_MNSS_CLIENT_SS_SRV_REL)
            {
                ...
                switch (srvRel_rsp->type)
                {
                    case SS_SRV_TYPE_FORWARDING_INFORMATION:
                    {
                        ...
                        if( (rdata->class_size == 1)
                             && (rdata->call_forward_class_info_list[0].activated == 0)

...
void KRIL_Capi2HandleRespCbk(UInt32 tid, UInt8 clientID, MsgType_t msgType, SimNumber_t SimId, UInt32 DialogId, Result_t result, void *dataBuf, UInt32 dataLength, ResultDataBufHandle_t dataBufHandle)
{
    ...
    else if (MSG_REG_GSM_IND == capi2_rsp->msgType && KRIL_GetPreferredNetworkTID() != 0)  // Change the notify message to response message for the MSG_REG_GSM_IND
    {
        capi2_rsp->tid = KRIL_GetPreferredNetworkTID();
    }
    else if (MSG_REG_GSM_IND == capi2_rsp->msgType && KRIL_GetConfigEmergencyIdleModeTID() != 0)  // Change the notify message to response message for the MSG_REG_GSM_IND
    {
        capi2_rsp->tid = KRIL_GetConfigEmergencyIdleModeTID();
    }

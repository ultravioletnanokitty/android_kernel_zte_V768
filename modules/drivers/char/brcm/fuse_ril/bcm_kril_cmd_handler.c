...
static Boolean sFlightModeStatus[DUAL_SIM_SIZE] = {TRUE, TRUE, TRUE};
...
static UInt32 sPrferredNetworkTID = 0;
...
static UInt32 sConfigEmergencyIdleModeTID = 0;
...
kril_capi2_handler_fn_t g_kril_capi2_handler_array[]=
{
     ...
     {BRCM_RIL_REQUEST_SETUP_DATA_CALL, KRIL_SetupPdpHandler, sizeof(UInt8)},
     //lvjinlong@wind-mobi.com start 2012.03.21
     ...
     {BRCM_RIL_REQUEST_UICC_SELECT_APPLICATION,KRIL_SimOpenChannelHandler,0},
     {BRCM_RIL_REQUEST_UICC_DEACTIVATE_APPLICATION,KRIL_SimCloseChannelHandler,sizeof(int)},
     {BRCM_RIL_REQUEST_UICC_APPLICATION_IO,KRIL_UiccApplicationIOHandler,0},
     {BRCM_RIL_REQUEST_UICC_AKA_AUTHENTICATE,BRIL_IsimAkaAuthenticationHandler,0},   
     {BRCM_RIL_REQUEST_UICC_GBA_AUTHENTICATE_BOOTSTRAP,BRIL_IsimGbaAuthenticateBootstrapHandler,0},
     {BRCM_RIL_REQUEST_UICC_GBA_AUTHENTICATE_NAF,BRIL_IsimGbaAuthenticateNafHandler,0},     
     ...
     {RIL_REQUEST_AGPS_OPEN_PKCS15, KRIL_AgpsOpenPkcs15Handler, sizeof(AgpsOpenPkcs15Cxt)},
     {RIL_REQUEST_AGPS_SIM_EF_INFO, KRIL_AgpsSimEfInfoHandler, sizeof(AgpsEfInfoCxt)},
     {RIL_REQUEST_AGPS_SIM_READ_REC, KRIL_AgpsSimRecHandler, 0},
     {RIL_REQUEST_AGPS_SIM_READ_EF, KRIL_AgpsSimEfHandler, 0},
	 {RIL_REQUEST_AGPS_CLOSE_PKCS15, KRIL_AgpsClosePkcs15Handler, 0},
...
void KRIL_SendResponse(KRIL_CmdList_t *listentry)
{
	...
	//lvjinlong@wind-mobi.com start 2010.06.21
	//csp patch 538696
    //if (0 == entry->result_info.datalen)
    if (0 == entry->result_info.datalen || NULL == listentry->bcm_ril_rsp)
	//lvjinlong@wind-mobi.com end 2010.06.21
    {
...
void KRIL_SendNotify(SimNumber_t SimId, int CmdID, void *rsp_data, UInt32 rsp_len)
{
	...
	//csp 538696 patch 
	//if (0 == entry->result_info.datalen)
    if (0 == entry->result_info.datalen || NULL == rsp_data)
	//lvjinlong@wind-mobi.com start 2012.06.21
    {
        entry->result_info.data = NULL;
    }
...
void KRIL_CommandThread(struct work_struct *data)
{
    ...
    list_for_each_safe(listptr, pos, &gKrilCmdQueue.list)
    {
        ...
        KRIL_DEBUG(DBG_TRACE, "entry cmd:%ld list:%p next:%p prev:%p\n", entry->cmd, &entry->list, entry->list.next, entry->list.prev);
         if (entry->ril_cmd->SimId < SIM_SINGLE || entry->ril_cmd->SimId > SIM_DUAL_SECOND) {
             KRIL_DEBUG(DBG_ERROR, "%s() Invalid SimId %d\n", __FUNCTION__, entry->ril_cmd->SimId);
             list_del(listptr);
             ...
             if(cmd_list) {
                 ...
                 cmd_list->result = BCM_E_ILLEGAL_SIM_OR_ME;
                 cmd_list->handler_state = BCM_SendCAPI2Cmd;
                 KRIL_SendResponse(cmd_list);
                 kfree(cmd_list);
                 cmd_list = NULL;
             }
             if (entry->ril_cmd->data != NULL && entry->ril_cmd->datalen != 0)
                 kfree(entry->ril_cmd->data);
             kfree(entry->ril_cmd);
             kfree(entry);
             continue;
        }
        ...
        if((TRUE == found) && (i < ARRAY_SIZE(g_kril_capi2_handler_array)))
        {
            ...
            {
                KRIL_DEBUG(DBG_TRACE, "handler_state:%ld CmdID:%ld result:%d\n", cmd_list->handler_state, cmd_list->ril_cmd->CmdID, cmd_list->result);
                KRIL_SendResponse(cmd_list);
        ...
        if(TRUE == found)
        {
            ...
            if(BCM_FinishCAPI2Cmd == listentry->handler_state || BCM_ErrorCAPI2Cmd == listentry->handler_state)
            {
                ...
                if (BCM_URIL_CLIENT == listentry->ril_cmd->client ||
					...
					BCM_URIL_AGPS_CLIENT == listentry->ril_cmd->client)
                {
            ...
            else
            {
                if (!(entry->msgType == MSG_SYS_ENABLE_CELL_INFO_RSP || // Skip to update the TID for enable cell information
                     ...
                     ((entry->msgType == MSG_MS_SET_RAT_BAND_RSP ||entry->msgType == MSG_REG_GSM_IND) && BCM_NET_SendChangeRAT == listentry->handler_state) || // Skip to update the TID for set preferred RAT.
                     (entry->msgType == MSG_REG_GSM_IND && listentry->cmd == BRCM_RIL_REQUEST_OEM_HOOK_RAW && listentry->handler_state == BCM_Wait_GSM_NO_Service) ||
...
Boolean IsNeedToWait(SimNumber_t SimId, unsigned long CmdID)
{
            ...
            BRCM_RIL_REQUEST_RADIO_POWER == CmdID)
    {
        ...
        return found;
            ...
            BRCM_RIL_REQUEST_CONFERENCE == CmdID ||
            BRCM_RIL_REQUEST_HANGUP == CmdID ||
            BRCM_RIL_REQUEST_EXPLICIT_CALL_TRANSFER == CmdID ) // Just only send one MPTY call request to CP to avoid call state error
    {
        struct list_head *listptr, *listpos;
        KRIL_CmdList_t *listentry = NULL;
        Boolean found = FALSE;
        ...
        list_for_each_safe(listptr, listpos, &gKrilCmdList.list)
        {
            ...
            if(BRCM_RIL_REQUEST_HANGUP_WAITING_OR_BACKGROUND == listentry->ril_cmd->CmdID ||
               ...
               BRCM_RIL_REQUEST_EXPLICIT_CALL_TRANSFER == listentry->ril_cmd->CmdID)
            {
                KRIL_DEBUG(DBG_INFO, "command list::CmdID:%ld find CmdID:%ld tid:%ld\n", CmdID, listentry->ril_cmd->CmdID, listentry->tid);
                found = TRUE;
                break;
...
//luobiao@wind-mobi.com start 2012.05.17
//patch for csp:524605
// Function Name: KRIL_SetFlightModeState
//
// Description:   Store the flight mode status.
//
// Notes:
//
//******************************************************************************
void KRIL_SetFlightModeState(SimNumber_t SimId, Boolean onoff)
{
    KRIL_DEBUG(DBG_INFO,"SIM%d sFlightModeStatus:%d\n", SimId, onoff);
    sFlightModeStatus[SimId] = onoff;
}

//******************************************************************************
//
// Function Name: KRIL_GetFlightModeState
//
// Description:   Get the flight mode status.
//
// Notes:
//
//******************************************************************************
Boolean KRIL_GetFlightModeState(SimNumber_t SimId)
{
    KRIL_DEBUG(DBG_INFO,"SIM%d sFlightModeStatus:%d\n", SimId, sFlightModeStatus[SimId]);
    return sFlightModeStatus[SimId];
}
...
//******************************************************************************
//
// Function Name: KRIL_SetPreferredNetworkTID
//
// Description:   Store the TID for KRIL_SetPreferredNetworkTypeHandler requests.
//
// Notes:
//
//******************************************************************************
void KRIL_SetPreferredNetworkTID(UInt32 tid)
{
    KRIL_DEBUG(DBG_INFO,"sPrferredNetworkTID:%ld\n", tid);
    sPrferredNetworkTID = tid;
}

//******************************************************************************
//
// Function Name: KRIL_GetPreferredNetworkTID
//
// Description:   Get the TID for KRIL_SetPreferredNetworkTypeHandler requests.
//
// Notes:
//
//******************************************************************************
UInt32 KRIL_GetPreferredNetworkTID(void)
{
    KRIL_DEBUG(DBG_INFO,"sPrferredNetworkTID:%ld\n", sPrferredNetworkTID);
    return sPrferredNetworkTID;
}
...
//******************************************************************************
//
// Function Name: KRIL_SetConfigEmergencyIdleModeTID
//
// Description:   Store the TID for KRIL_ConfigEmergencyIdleMode requests.
//
// Notes:
//
//******************************************************************************
void KRIL_SetConfigEmergencyIdleModeTID(UInt32 tid)
{
    KRIL_DEBUG(DBG_INFO,"KRIL_SetConfigEmergencyIdleModeTID:%ld\n", tid);
    sConfigEmergencyIdleModeTID = tid;
}

//******************************************************************************
//
// Function Name: KRIL_GetConfigEmergencyIdleModeTID
//
// Description:   Get the TID for KRIL_ConfigEmergencyIdleMode requests.
//
// Notes:
//
//******************************************************************************
UInt32 KRIL_GetConfigEmergencyIdleModeTID(void)
{
    KRIL_DEBUG(DBG_INFO,"KRIL_GetConfigEmergencyIdleModeTID:%ld\n", sConfigEmergencyIdleModeTID);
    return sConfigEmergencyIdleModeTID;
}
        ...
        rawdata[3] == 'M')
    {
        ...
        switch (msgid)
        {
           ...
           case BRIL_HOOK_SET_PDP_ACTIVATION_FDN_CONTROL:
	        ...
	        KRIL_DEBUG(DBG_TRACE,"KRIL_OemHookRawHandler=>BRIL_HOOK_QUERY_NETWORK_LOCK\n");
                KRIL_QueryNetworkLockHandler(ril_cmd, capi2_rsp);
                break;
            
            case BRIL_HOOK_QUERY_NETWORK_SUBSET_LOCK:
	        KRIL_DEBUG(DBG_TRACE,"KRIL_OemHookRawHandler=>BRIL_HOOK_QUERY_NETWORK_SUBSET_LOCK\n");
                KRIL_QueryNetworkSubsetLockHandler(ril_cmd, capi2_rsp);
                break;

            case BRIL_HOOK_QUERY_SERVICE_PROVIDE_LOCK:
	        KRIL_DEBUG(DBG_TRACE,"KRIL_OemHookRawHandler=>BRIL_HOOK_QUERY_SERVICE_PROVIDE_LOCK\n");
                KRIL_QueryServiceProvideLockHandler(ril_cmd, capi2_rsp);
                break;

            case BRIL_HOOK_QUERY_CORPORATE_LOCK:
	        KRIL_DEBUG(DBG_TRACE,"KRIL_OemHookRawHandler=>BRIL_HOOK_QUERY_CORPORATE_LOCK\n");
                KRIL_QueryCorporateLockHandler(ril_cmd, capi2_rsp);
                break;

            case BRIL_HOOK_QUERY_PHONE_LOCK:
	        KRIL_DEBUG(DBG_TRACE,"KRIL_OemHookRawHandler=>BRIL_HOOK_QUERY_PHONE_LOCK\n");
            ...
            case BRIL_HOOK_SET_EMERGENCY_IDLE_MODE_ELEMENT:
                KRIL_ConfigEmergencyIdleMode(ril_cmd, capi2_rsp);
                break;

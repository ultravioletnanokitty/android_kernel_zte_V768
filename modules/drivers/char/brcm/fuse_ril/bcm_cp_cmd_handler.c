...
void CAPI2_FFS_Control(UInt32 tid, UInt8 clientID, UInt32 cmd, UInt32 address, UInt32 offset, UInt32 size)
{
    ...
    UInt32 fsize;
//songjinguo@wind-mobi.com 2011.01.18 begin
//fix bug 6962; CSP: 492589; register 2G network is slow
//review by liubing
    ...
    set_fs(KERNEL_DS);
    char* tmpBuf = NULL;
//songjinguo@wind-mobi.com 2011.01.18 end            
...
//fix bug 6962; CSP: 492589; register 2G network is slow
//review by liubing	            
	         // retval = hFileTmp->f_op->read(hFileTmp, pPersistDataAddr+offset, size, &hFileTmp->f_pos);
...
//review by liubing	             
		   ...
		   //retval = hFileTmp->f_op->write(hFileTmp, pPersistDataAddr+offset, size, &hFileTmp->f_pos);	

...
#define CK_MAX_LENGTH		  16 ///< Max. number of digits in Control Key of SIMLock
...
//ISIM authenication
#define MAX_AUTH_DATA_LENGTH	    256
#define MAX_AUTH_RES_LEN	    200 	///< Maximum length for RES data
#define MAX_AUTH_CK_LEN 	    200 	///< Maximum length for CK data
#define MAX_AUTH_IK_LEN 	    200 	///< Maximum length for IK data
#define MAX_AUTH_AUTS_LEN	    255 	///< Maximum length for AUTS data
#define MAX_AUTH_KS_NAF_LEN         200 	///< Maximum length for KS EXT NAF data
...
typedef struct {
    char password[CK_MAX_LENGTH+1];
    char newpassword[CK_MAX_LENGTH+1];
} KrilSimPinNum_t;
...
typedef struct {
    int  session_id; /* The session_id of the app */
    int  rand_len; 
    char rand[MAX_AUTH_DATA_LENGTH];
    int  autn_len; 
    char autn[MAX_AUTH_DATA_LENGTH]; 
} KrilIsimAkaAuthenticate_t;

typedef struct {
    char res[MAX_AUTH_RES_LEN*2+1];
    char ck[MAX_AUTH_CK_LEN*2+1];
    char ik[MAX_AUTH_IK_LEN*2+1];
    char auts[MAX_AUTH_AUTS_LEN*2+1];
} KrilIsimAkaAuthenticateRsp_t;

//ISIM GBA Authenticate Bootstrap
typedef struct {
    int  session_id; /* The session_id of the app */
    int  rand_len; 
    char rand[MAX_AUTH_DATA_LENGTH];
    int  autn_len; 
    char autn[MAX_AUTH_DATA_LENGTH]; 
} KrilIsimGbaAuthBootstrap_t;

typedef struct {
    char res[MAX_AUTH_RES_LEN*2+1];
    char auts[MAX_AUTH_AUTS_LEN*2+1];
} KrilIsimGbaAuthBootstrapRsp_t;

//ISIM GBA Authenticate NAF

typedef struct {
    int  session_id; /* The session_id of the app */
    int  naf_id_len; 
    char naf_id[MAX_AUTH_DATA_LENGTH];
} KrilIsimGbaAuthentateNaf_t;

typedef struct {
    char ks_naf_data[MAX_AUTH_KS_NAF_LEN*2+1];
} KrilIsimGbaAuthentateNafRsp_t;
...
/* yaogangxiang@wind-mobi.com2011.12.27 begin */
//add simlock patch,simlock_APpatchforHOOKmsgType.zip
typedef enum {
    ...
    BRIL_HOOK_SET_EMERGENCY_IDLE_MODE_ELEMENT    //config MS_LOCAL_PHCTRL_ELEM_EMERGENCY_IDLE_MODE element
} HOOK_msgType;
/* yaogangxiang@wind-mobi.com 2011.12.27 end */
...
#define BRCM_RIL_REQUEST_UICC_SELECT_APPLICATION 104	  
#define BRCM_RIL_REQUEST_UICC_DEACTIVATE_APPLICATION 105
#define BRCM_RIL_REQUEST_UICC_APPLICATION_IO 106
#define BRCM_RIL_REQUEST_UICC_AKA_AUTHENTICATE 107
#define BRCM_RIL_REQUEST_UICC_GBA_AUTHENTICATE_BOOTSTRAP 108
#define BRCM_RIL_REQUEST_UICC_GBA_AUTHENTICATE_NAF 109
#define BRCM_RIL_REQUEST_PDP_ACTIVATION_NWI_IND_REJECT 110
...
/**
 * RIL_REQUEST_AGPS_OPEN_PKCS15
 *
 * Open the PKCS#15 file in SIM
 *
 * "data" is NULL
 *
 * "response" is unsigned char. PKCS#15 handle. 0xff is unsupported.
 *
 * Valid errors:
 *  SUCCESS
 *  RADIO_NOT_AVAILABLE
 *  GENERIC_FAILURE
 */
#define RIL_REQUEST_AGPS_OPEN_PKCS15				RIL_REQUEST_AGPS_BASE+4
 ...
 * Get the EF info
 *
 * "data" is AgpsEfInfoReq *
 *
 * "response" is AgpsEfInfoRes
 *
 * Valid errors:
 *  SUCCESS
 *  RADIO_NOT_AVAILABLE
 *  GENERIC_FAILURE
 */
#define RIL_REQUEST_AGPS_SIM_EF_INFO				RIL_REQUEST_AGPS_BASE+5

/**
 * RIL_REQUEST_AGPS_SIM_READ_REC
 *
 * Get the EF info
 *
 * "data" is AgpsEfRecReq *
 *
 * "response" is AgpsRecData
 *
 * Valid errors:
 *  SUCCESS
 *  RADIO_NOT_AVAILABLE
 *  GENERIC_FAILURE
 */
#define RIL_REQUEST_AGPS_SIM_READ_REC				RIL_REQUEST_AGPS_BASE+6

/**
 * RIL_REQUEST_AGPS_SIM_READ_EF
 *
 * Get the EF info
 *
 * "data" is AgpsEfReadReq *
 *
 * "response" is AgpsEfData
 *
 * Valid errors:
 *  SUCCESS
 *  RADIO_NOT_AVAILABLE
 *  GENERIC_FAILURE
 */
#define RIL_REQUEST_AGPS_SIM_READ_EF				RIL_REQUEST_AGPS_BASE+7
 ...
 * Open the PKCS#15 file in SIM
 *
 * "data" is unsigned char, PKCS#15 handle
 *
 * "response" is NULL
 *
 * Valid errors:
 ...
 *  GENERIC_FAILURE
 */
#define RIL_REQUEST_AGPS_CLOSE_PKCS15				RIL_REQUEST_AGPS_BASE+8
...
typedef struct 
{
	unsigned char 	pkcs15Hdl;
	unsigned short	path;
} AgpsEfInfoReq;

typedef enum
{
  AGPS_SIM_APPL_2G,		///< 2G SIM is inserted
  AGPS_SIM_APPL_3G,		///< 3G USIM is inserted
  AGPS_SIM_APPL_INVALID	///< No SIM is inserted
} AgpsSimType;	///< SIM application type
...
{
	unsigned short	path;
	unsigned char	recIdx;
	unsigned char	numOfRec;
...
typedef struct 
{
	int				recLen;
	int				dataLen;
	unsigned char	data[BRCM_AGPS_MAX_MESSAGE_SIZE];
} AgpsRecData;
...
typedef struct 
{
	unsigned char 	pkcs15Hdl; //socket ID
	unsigned short	appPath;
	unsigned short	subPath;
	unsigned char	idx;
	unsigned short 	len;
} AgpsEfReadReq;

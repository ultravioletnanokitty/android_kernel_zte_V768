...
Boolean gIsSimCachedDataReady[DUAL_SIM_SIZE] = {FALSE, FALSE, FALSE};
...
int ParseIsIsimSupported(KRIL_CmdList_t *pdata, Kril_CAPI2Info_t *capi2_rsp)
{
	Boolean* rsp = (Boolean*)capi2_rsp->dataBuf;
	KrilSimStatusResult_t *p_sim_status = (KrilSimStatusResult_t *)pdata->bcm_ril_rsp;
	...
	if (!p_sim_status)
	{
		KRIL_DEBUG(DBG_ERROR,"p_sim_status is NULL. Error!!!\n");
		return 0;
	}

	if (1 == *rsp)
	{
		KRIL_DEBUG(DBG_INFO,"SIM[%d]: ISIM is supported\n",pdata->ril_cmd->SimId);
		p_sim_status->isim_supported = 1;
	}
	else
	{
		KRIL_DEBUG(DBG_INFO,"SIM[%d]: ISIM is not supported\n",pdata->ril_cmd->SimId);
		p_sim_status->isim_supported = 0;
	}
...
void ConvertISIMSelectRspformatToSIM(KRIL_CmdList_t *pcmd,
                                     SIM_RESTRICTED_ACCESS_DATA_t *rsp,
                                     UInt8 *data,
                                     UInt16 *data_len
                                     )
{
               KrilSimApplicationIOCmd_t *cmd_data = (KrilSimApplicationIOCmd_t*)pcmd->ril_cmd->data;
               int fileid = cmd_data->fileid;
               UInt8 *pdata = rsp->data;
               UInt16 pdata_len = rsp->data_len;
               UInt16 i;
...
void ParseUICCRestrictedAccessData(KRIL_CmdList_t *pdata, Kril_CAPI2Info_t *capi2_rsp)
{
    SIM_RESTRICTED_ACCESS_DATA_t *rsp = (SIM_RESTRICTED_ACCESS_DATA_t*)capi2_rsp->dataBuf;
    KrilSimApplicationIOCmd_t *cmd_data = (KrilSimApplicationIOCmd_t*)pdata->ril_cmd->data;
    KrilSimIOResponse_t *simioresult;
    ...
    if (rsp->result != SIMACCESS_SUCCESS)
    {
        KRIL_DEBUG(DBG_ERROR,"UICC IO RSP: SIM access failed!! result:%d\n", rsp->result);
        simioresult->result = BCM_E_GENERIC_FAILURE;
        pdata->handler_state = BCM_ErrorCAPI2Cmd;
        return;
    }
	
    KRIL_DEBUG(DBG_INFO,"UICC IO: command:%d fileid:0x%X SW1:0x%X SW2:0x%X DataLen:%d\n",
                    cmd_data->command, cmd_data->fileid, rsp->sw1, rsp->sw2, rsp->data_len);
    ...
    {
        //print the SIM data
        //RawDataPrintfun(rsp->data, rsp->data_len, "UICC IO RSP");

        if (rsp->data_len > 256)
        {
            // If this issue happen, modify the buffer length of 
            // simResponse[] in KrilSimIOResponse_t
            KRIL_DEBUG(DBG_ERROR,"UICCIO RSP: SIM response length is too long:%d\n", rsp->data_len);
            simioresult->result = BCM_E_GENERIC_FAILURE;
            pdata->handler_state = BCM_ErrorCAPI2Cmd;
            return;
        }
			...
			APDUCMD_GET_RESPONSE == cmd_data->command)
	{
		...
		ConvertISIMSelectRspformatToSIM(pdata, rsp, data, &data_len);
		//RawDataPrintfun(data, data_len, "CONVERT SIMIO");
		HexDataToHexStr(simioresult->simResponse, data, data_len);
	}
	else
...
void KRIL_GetSimStatusHandler(void *ril_cmd, Kril_CAPI2Info_t *capi2_rsp)
{
    ...
    switch (pdata->handler_state)
    {
        ...
        case BCM_IS_PIN2_ENABLE:
            ...
            pdata->handler_state = BCM_IS_ISIM_SUPPORTED;
            break;
        
        case BCM_IS_ISIM_SUPPORTED:
            ...
            // Check if ISIM is supported
            KRIL_DEBUG(DBG_INFO,"gIsSimCachedDataReady[%d]:%d\n",
                pdata->ril_cmd->SimId, gIsSimCachedDataReady[pdata->ril_cmd->SimId]);
            
            if (TRUE == gIsSimCachedDataReady[pdata->ril_cmd->SimId])
            {
                CAPI2_ISimApi_IsIsimSupported(InitClientInfo(pdata->ril_cmd->SimId));
        ...
        case BCM_RESPCAPI2Cmd:
            ...
            if (!ParseIsIsimSupported(pdata, capi2_rsp))
            {
                pdata->handler_state = BCM_ErrorCAPI2Cmd;
                return;
            }            
...
void KRIL_SimOpenChannelHandler(void *ril_cmd, Kril_CAPI2Info_t *capi2_rsp)
{
    ...
    switch (pdata->handler_state)
    {
        case BCM_SendCAPI2Cmd:
        {
            ...
            CAPI2_ISimApi_ActivateIsimAppli(InitClientInfo(pdata->ril_cmd->SimId));
        ...
        case BCM_RESPCAPI2Cmd:
        {
            ISIM_ACTIVATE_RSP_t *pIsimActRes = (ISIM_ACTIVATE_RSP_t*)capi2_rsp->dataBuf;
            int socket_id = 0;
            ...
            if (SIMACCESS_SUCCESS == pIsimActRes->result)
            {
                KRIL_DEBUG(DBG_INFO,"Acitvate ISIM[%d] successfully: socket_id:%d\n", 
                        pdata->ril_cmd->SimId, pIsimActRes->socket_id);
                socket_id = pIsimActRes->socket_id;
                memcpy(pdata->bcm_ril_rsp,&socket_id,pdata->rsp_len);
                pdata->handler_state = BCM_FinishCAPI2Cmd;

            }
            else
            {
                KRIL_DEBUG(DBG_ERROR,"Acitvate ISIM[%d] Failed!!! result:%d\n", 
                        pdata->ril_cmd->SimId, pIsimActRes->result);
                pdata->handler_state = BCM_ErrorCAPI2Cmd;
                return;
            }
            break;
...
void KRIL_SimCloseChannelHandler(void *ril_cmd, Kril_CAPI2Info_t *capi2_rsp)
{
   ...
   KRIL_DEBUG(DBG_INFO,"pdata->handler_state:0x%lX\n", pdata->handler_state);
    ...
    if (!pdata->cmdContext)
    {
        KRIL_DEBUG(DBG_ERROR,"pdata->cmdContext is NULL. Error!!!\n")
        pdata->handler_state = BCM_ErrorCAPI2Cmd;
        return;
    }
    ...
    switch (pdata->handler_state)
    {
        ...
        {
            int socket_id = 0;
            if (!pdata->ril_cmd->data)
            {
                KRIL_DEBUG(DBG_ERROR,"pdata->ril_cmd->data is NULL. Error!!!\n");
                pdata->handler_state = BCM_ErrorCAPI2Cmd;
                return;
            }

            socket_id = *((int*)pdata->ril_cmd->data);
            KRIL_DEBUG(DBG_ERROR,"socket_id =0x%x !\n",socket_id);
            
            if ((socket_id <= 0) || (socket_id > 3))
            {
                KRIL_DEBUG(DBG_ERROR,"Socket ID:0x%x is invalid.\n",socket_id);
                pdata->handler_state = BCM_ErrorCAPI2Cmd;
                return;
            }
			
            *(int*)pdata->cmdContext = (int)socket_id;

            CAPI2_SimApi_SendDeactivateAppiReq(InitClientInfo(pdata->ril_cmd->SimId),(UInt8)socket_id);
            pdata->handler_state = BCM_SIM_CloseSocket;
            break;
        }

        case BCM_SIM_CloseSocket:
        {
            SIM_DEACTIVATE_APPLI_RES_t *app_rsp = NULL;
            UInt8 socket_id = 0;			

            if (!capi2_rsp->dataBuf)
            {
                KRIL_DEBUG(DBG_ERROR,"capi2_rsp->dataBuf is NULL. Error!!!\n");
                pdata->handler_state = BCM_ErrorCAPI2Cmd;
                return;
            ...
            socket_id = *(UInt8*)pdata->cmdContext;
            app_rsp = (SIM_DEACTIVATE_APPLI_RES_t*)capi2_rsp->dataBuf;

            if(app_rsp->result != SIMACCESS_SUCCESS)
            {
                pdata->handler_state = BCM_ErrorCAPI2Cmd;
                KRIL_DEBUG(DBG_ERROR,"DeActiving channel(0x%x) is not success.Result:%d!!!\n",socket_id,app_rsp->result);
                return;
            }

            CAPI2_SimApi_SendCloseSocketReq(InitClientInfo(pdata->ril_cmd->SimId),socket_id);
            pdata->handler_state = BCM_RESPCAPI2Cmd;
            break;
        }
        ...
        case BCM_RESPCAPI2Cmd:
        {
            SIM_CLOSE_SOCKET_RES_t *close_rsp = NULL;
            if (!capi2_rsp->dataBuf)
            {
                KRIL_DEBUG(DBG_ERROR,"capi2_rsp->dataBuf is NULL. Error!!!\n");
                pdata->handler_state = BCM_ErrorCAPI2Cmd;
            ...
            close_rsp = (SIM_CLOSE_SOCKET_RES_t*)capi2_rsp->dataBuf;
            if(close_rsp->result != SIMACCESS_SUCCESS)
            {
                pdata->handler_state = BCM_ErrorCAPI2Cmd;
                KRIL_DEBUG(DBG_ERROR,"Close Socket is not success!Result(%d)\n",close_rsp->result);
                return;
            }
...
void KRIL_UiccApplicationIOHandler(void *ril_cmd, Kril_CAPI2Info_t *capi2_rsp)
{
    ...
    switch (pdata->handler_state)
    {
        case BCM_SendCAPI2Cmd:
        {
            ...
            KRIL_DEBUG(DBG_INFO,"UICC IO: session_id:%d command:%d fileid:0x%X DFid:0x%X p1:%d p2:%d p3:%d pin2:%s\n",
                    cmd_data->session_id,cmd_data->command, cmd_data->fileid, cmd_data->dfid, cmd_data->p1, cmd_data->p2, cmd_data->p3,
                    cmd_data->pin2);
            RawDataPrintfun((UInt8*)cmd_data->path, cmd_data->pathlen*2, "UICC IO: PATH");

			
            if(cmd_data->session_id == 0)
            {
                cmd_data->dfid = APDUFILEID_USIM_ADF;
            }
            else
            {
                cmd_data->dfid = APDUFILEID_ISIM_ADF;
            }
            ...
            CAPI2_SimApi_SubmitRestrictedAccessReq(InitClientInfo(pdata->ril_cmd->SimId), (UInt8)cmd_data->session_id,
                        (APDUCmd_t)cmd_data->command, efile_id, dfile_id, (UInt8)cmd_data->p1, (UInt8)cmd_data->p2,
                        (UInt8)cmd_data->p3, cmd_data->pathlen, cmd_data->path, (UInt8*)cmd_data->data);
        ...
        case BCM_RESPCAPI2Cmd:
        {
            ParseUICCRestrictedAccessData(pdata, capi2_rsp);
            break;
        }
...
void BRIL_IsimAkaAuthenticationHandler(void *ril_cmd, Kril_CAPI2Info_t *capi2_rsp)
{
    ...
    switch (pdata->handler_state)
    {
        case BCM_SendCAPI2Cmd:
        {
            KrilIsimAkaAuthenticate_t *cmd_data = NULL;
            ...
            if (!pdata->ril_cmd->datalen)
            {
                KRIL_DEBUG(DBG_ERROR,"pdata->ril_cmd->datalen is 0. Error!!!\n");
                pdata->handler_state = BCM_ErrorCAPI2Cmd;
                return;
            }
            cmd_data = (KrilIsimAkaAuthenticate_t*)pdata->ril_cmd->data;
			
            if ((cmd_data->session_id <= 0) || (cmd_data->session_id > 3))
            {
                KRIL_DEBUG(DBG_ERROR,"Session ID:0x%x is invalid.\n",cmd_data->session_id);
                pdata->handler_state = BCM_ErrorCAPI2Cmd;
                return;
            }
            RawDataPrintfun(cmd_data->rand, cmd_data->rand_len, "Isim Aka rand:");
            RawDataPrintfun(cmd_data->autn, cmd_data->autn_len, "Isim Aka autn:");
            CAPI2_ISimApi_SendAuthenAkaReq(InitClientInfo(pdata->ril_cmd->SimId), 
                        cmd_data->rand, cmd_data->rand_len, cmd_data->autn, cmd_data->autn_len);
        ...
        case BCM_RESPCAPI2Cmd:
        {
            ISIM_AUTHEN_AKA_RSP_t *aka_rsp = NULL;
            KrilIsimAkaAuthenticateRsp_t *IsimAka = NULL;
            if (!capi2_rsp->dataBuf)
            {
                KRIL_DEBUG(DBG_ERROR,"capi2_rsp->dataBuf is NULL. Error!!!\n");
                pdata->handler_state = BCM_ErrorCAPI2Cmd;
            ...
            aka_rsp = (ISIM_AUTHEN_AKA_RSP_t*)capi2_rsp->dataBuf;

            pdata->rsp_len = sizeof(KrilIsimAkaAuthenticateRsp_t);
            ...
            IsimAka = pdata->bcm_ril_rsp;	
			
            if (ISIM_AUTHEN_AKA_STATUS_SUCCESS == aka_rsp->aka_status)
            {
                HexDataToHexStr(IsimAka->res, aka_rsp->aka_data.aka_success.res_data, aka_rsp->aka_data.aka_success.res_len);
                HexDataToHexStr(IsimAka->ck, aka_rsp->aka_data.aka_success.ck_data, aka_rsp->aka_data.aka_success.ck_len);			
                HexDataToHexStr(IsimAka->ik, aka_rsp->aka_data.aka_success.ik_data, aka_rsp->aka_data.aka_success.ik_len);
                KRIL_DEBUG(DBG_INFO,"AKA Success: res:%s, ck:%s, ik:%s \n",IsimAka->res,IsimAka->ck,IsimAka->ik);
				
                pdata->handler_state = BCM_FinishCAPI2Cmd;
            }
            else if (ISIM_AUTHEN_AKA_STATUS_SYNC_FAIL == aka_rsp->aka_status)
            {
                HexDataToHexStr(IsimAka->auts, aka_rsp->aka_data.aka_sync_failure.auts_data, aka_rsp->aka_data.aka_sync_failure.auts_len);
                KRIL_DEBUG(DBG_INFO,"AKA Failure: auts:%s \n",IsimAka->auts);
                pdata->handler_state = BCM_FinishCAPI2Cmd;
            }
            else
            {
                pdata->handler_state = BCM_ErrorCAPI2Cmd;
                KRIL_DEBUG(DBG_ERROR,"Error:aka_status:%d\n",aka_rsp->aka_status);
            }
            break;
        }
...
void BRIL_IsimGbaAuthenticateBootstrapHandler(void *ril_cmd, Kril_CAPI2Info_t *capi2_rsp)
{
    ...
    switch (pdata->handler_state)
    {
        case BCM_SendCAPI2Cmd:
        {
            KrilIsimGbaAuthBootstrap_t *cmd_data = NULL;
            ...
            cmd_data = (KrilIsimGbaAuthBootstrap_t*)pdata->ril_cmd->data;
            ...
            RawDataPrintfun(cmd_data->rand, cmd_data->rand_len, "Isim Gbaboot rand:");
            RawDataPrintfun(cmd_data->autn, cmd_data->autn_len, "Isim Gbaboot autn:");			
            CAPI2_ISimApi_SendAuthenGbaBootReq(InitClientInfo(pdata->ril_cmd->SimId), 
                        cmd_data->rand, cmd_data->rand_len, cmd_data->autn, cmd_data->autn_len);
        ...
        case BCM_RESPCAPI2Cmd:
        {
            ISIM_AUTHEN_GBA_BOOT_RSP_t *gbaboot_rsp = NULL;
            KrilIsimGbaAuthBootstrapRsp_t *isim_gba_boot = NULL;
            if (!capi2_rsp->dataBuf)
            {
                KRIL_DEBUG(DBG_ERROR,"capi2_rsp->dataBuf is NULL. Error!!!\n");
                pdata->handler_state = BCM_ErrorCAPI2Cmd;
            ...
            gbaboot_rsp = (ISIM_AUTHEN_GBA_BOOT_RSP_t*)capi2_rsp->dataBuf;

            pdata->rsp_len = sizeof(KrilIsimGbaAuthBootstrapRsp_t);
            ...
            isim_gba_boot = pdata->bcm_ril_rsp; 
			
            if (ISIM_AUTHEN_AKA_STATUS_SUCCESS == gbaboot_rsp->gba_boot_status)
            {
                HexDataToHexStr(isim_gba_boot->res, gbaboot_rsp->gba_boot_data.gba_boot_success.res_data,
                                gbaboot_rsp->gba_boot_data.gba_boot_success.res_len);

                KRIL_DEBUG(DBG_INFO,"Gba bootstrap Success: res:%s \n",isim_gba_boot->res);
                pdata->handler_state = BCM_FinishCAPI2Cmd;
            }
            else if (ISIM_AUTHEN_AKA_STATUS_SYNC_FAIL == gbaboot_rsp->gba_boot_status)
            {
                HexDataToHexStr(isim_gba_boot->auts, gbaboot_rsp->gba_boot_data.gba_boot_sync_failure.auts_data,
												  gbaboot_rsp->gba_boot_data.gba_boot_sync_failure.auts_len);
                KRIL_DEBUG(DBG_INFO,"Gba bootstrap Failure: auts:%s \n",isim_gba_boot->auts);
                pdata->handler_state = BCM_FinishCAPI2Cmd;
            }
            else
            {
                pdata->handler_state = BCM_ErrorCAPI2Cmd;
                KRIL_DEBUG(DBG_ERROR,"Error:gba_boot_status:%d\n",gbaboot_rsp->gba_boot_status);
            }
            break;
        }
...
void BRIL_IsimGbaAuthenticateNafHandler(void *ril_cmd, Kril_CAPI2Info_t *capi2_rsp)
{
    ...
    switch (pdata->handler_state)
    {
        case BCM_SendCAPI2Cmd:
        {
            KrilIsimGbaAuthentateNaf_t *cmd_data = NULL;
            ...
            cmd_data = (KrilIsimGbaAuthentateNaf_t*)pdata->ril_cmd->data;
            ...
            RawDataPrintfun(cmd_data->naf_id, cmd_data->naf_id_len, "Isim Gba Naf naf_id:");
			
            CAPI2_ISimApi_SendAuthenGbaNafReq(InitClientInfo(pdata->ril_cmd->SimId), 
                                    cmd_data->naf_id, cmd_data->naf_id_len);
        ...
        case BCM_RESPCAPI2Cmd:
        {
            ISIM_AUTHEN_GBA_NAF_RSP_t *gba_naf_rsp = NULL;
            KrilIsimGbaAuthentateNafRsp_t *isim_gba_naf = NULL;
            if (!capi2_rsp->dataBuf)
            {
                KRIL_DEBUG(DBG_ERROR,"capi2_rsp->dataBuf is NULL. Error!!!\n");
                pdata->handler_state = BCM_ErrorCAPI2Cmd;
            ...
            gba_naf_rsp = (ISIM_AUTHEN_GBA_NAF_RSP_t*)capi2_rsp->dataBuf;
            ...
            isim_gba_naf = pdata->bcm_ril_rsp;	
			
            if (ISIM_AUTHEN_AKA_STATUS_SUCCESS == gba_naf_rsp->gba_naf_status)
            {
                HexDataToHexStr(isim_gba_naf->ks_naf_data, gba_naf_rsp->gba_naf_success.ks_naf_data,
                                                gba_naf_rsp->gba_naf_success.ks_naf_len);
                KRIL_DEBUG(DBG_INFO,"Gba Naf Success: res:%s \n",isim_gba_naf->ks_naf_data);
                pdata->handler_state = BCM_FinishCAPI2Cmd;
            }
            else
            {
                pdata->handler_state = BCM_ErrorCAPI2Cmd;
                KRIL_DEBUG(DBG_ERROR,"Error:gba_Naf_status:%d\n",gba_naf_rsp->gba_naf_status);
           }			
           break;
        }

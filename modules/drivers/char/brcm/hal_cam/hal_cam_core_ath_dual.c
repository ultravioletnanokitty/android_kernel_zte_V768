...
//songjinguo@wind-mobi.com 20120324 start
//set 5M camera buffer
#ifdef CONFIG_BOARD_L400
#define CAM_BUFFER_SIZE_WIND (30 * 512 * 1024)
#else
#define CAM_BUFFER_SIZE_WIND (18 * 512 * 1024)
#endif
//songjinguo@wind-mobi.com 20120324 end
#define REFCAPTIME 80000000
//hanwei@wind-mobi.com chg 20120209 start 
//fix bug#7622 set wallpapers,camera hang
//liubing@wind-mobi.com
//#define SZ_BUFFER (1024 * 1024 * 6)
#define SZ_BUFFER (512 * 1024 * 8)
//hanwei@wind-mobi.com chg 20120209 end
UInt32 picture_storage_address;

#define FOCUS_MODE_ENABLE 1
#if (defined(CONFIG_BCM_CAM_S5K5CA)&& defined( CONFIG_BCM_CAM_OV7690)) //2012.03.02 hanwei for distinguish project 5M and 3M
 int camera_id;
#endif
#if (defined(CONFIG_BCM_CAM_OV5640)&& defined( CONFIG_BCM_CAM_OV7690)) //2012.03.02 hanwei for distinguish project 5M and 3M
 int camera_id;
#endif
struct clk *cam_clk;
...
//reviewed by liubing@wind-mobi.com
	if(c->mode==CAM_STILL)
		printk(KERN_ERR"------------- CAM_STILL  taskcallback---- 0x%x ", intr_status);
	else
		;//printk(KERN_ERR"++++++++++ CAM_STREAM  taskcallback---- 0x%x ", intr_status);
		     ...
		     unsigned long arg)
{
	...
	case CAM_IOCTL_SET_EXPOSURE:
		{
			printk("hanwei----------CAM_IOCTL_SET_EXPOSURE-------------called----\n");
			CAM_Parm_t parm;
			if (copy_from_user
			    (&parm, (CAM_Parm_t *) arg, sizeof(parm)) != 0) {
				rc = -EFAULT;
...
static int cam_power_up(CamSensorSelect_t sensor)
{
	int rc = -1;
	struct camera_sensor_t *c;
//hanwei@wind-mobi.com 2011.11.02 begin
//config camera voltage
#if defined(CONFIG_BOARD_THUNDERBIRD_EDN31) || defined(CONFIG_BOARD_THUNDERBIRD_EDN5x)  //hanwei
	if (cam_g->cam_regulator)
		rc = regulator_enable(cam_g->cam_regulator);
	       pr_debug("hanwei:**** 111111111rc = %d*****\n",rc);
	if (cam_g->cam_regulator1)
		rc = regulator_enable(cam_g->cam_regulator1);
	        pr_debug("hanwei:****222222222rc = %d*****\n",rc);
#endif
...
#endif
//hanwei chg-s  for remove boot up camera warning info,regulator_disable excess operation
//reviewed by yuanlan
#if 0//defined FOCUS_MODE_ENABLE 
	if (cam_g->cam_regulator_focus)
		rc = regulator_disable(cam_g->cam_regulator_focus);
#endif
//hanwei chg-e  for remove boot up camera warning info,regulator_disable excess operation	return 0;
...
//hanwei@wind-mobi.com chg 2011.11.04 end
		...
		if((c->zoom == CamZoom_1_0) || (c->main.format == CamDataFmtJPEG)){
			...
			if((sensor_size.resX <= 640) && (sensor_size.resY <= 480)){
				...
				cslCamBuffer1.start_addr = cam_g->cam_buf.phy + (1*1024*1024);
				//hanwei@wind-mobi.com chg 20120209 end
				size = sensor_size.resX * sensor_size.resY;
			}
			else {
...
#endif 
#if 0
//broadcom patch 2012-03-31
cslCamBuffer1.start_addr = cam_g->cam_buf.phy + (21 * 1024 * 1024) - \
					(c->main.size_window.end_pixel * c->main.size_window.end_line * 2)-(1*512*512);
#endif			
				cslCamBuffer1.start_addr = cam_g->cam_buf.phy + CAM_BUFFER_SIZE_WIND- (c->main.size_window.end_pixel * c->main.size_window.end_line * 2) -1024*1024;
				cslCamBuffer1.start_addr &= 0xFFFF0000;
				size = sensor_size.resX * sensor_size.resY;
			}
			one_buffer=1;
...
#ifdef CONFIG_BOARD_L400		//for L400 only, set one buffer;
		one_buffer=1; 	
#endif		
		if(one_buffer==1)
			P_Buffer2=NULL;
...
static int __init cam_init(void)
{
...
#if defined(CONFIG_BOARD_ACAR) || defined(CONFIG_BOARD_THUNDERBIRD_EDN31) || defined(CONFIG_BOARD_THUNDERBIRD_EDN5x) || defined(CONFIG_BOARD_PASCAL) || defined(CONFIG_BOARD_L400) 
	...
	//cam_power_up(cam_g->curr); 
#if 0//(defined(CONFIG_BCM_CAM_S5K5CA)&& defined( CONFIG_BCM_CAM_OV7690)) 

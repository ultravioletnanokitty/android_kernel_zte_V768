...
#define SZ_BUFFER (1024 * 512 * 8)  //5)  hanwei chg for chenggong merge patch lead to system halted.

UInt32 picture_storage_address;

#define CP_SHARED_MEM 0x80000000
#define CP_SHARED_MEM_SIZE (4*8)
...
struct semaphore fe_sem;
atomic_t shutdown_req  ;
...
extern void tp_homekey_control(int press);
...
static void taskcallback(UInt32 intr_status, UInt32 rx_status, UInt32 image_addr, UInt32 image_size, UInt32 raw_intr_status, UInt32 raw_rx_status, void *userdata)
{
	...
	t = ktime_get();
	int cur_dma_chan;
	//printk("--- coming in taskcallback 0x%x \n", intr_status);
	
	if((intr_status & CSL_CAM_INT_FRAME_START) && (intr_status & CSL_CAM_INT_FRAME_END)){
                printk("FS and FE **\n");
				atomic_add(1, &c->fefs );
				image_addr = 0;
				if(atomic_read(&c->fefs)>=25)
				{
					printk("home key pressed for FE FS \n");	
					tp_homekey_control(1);
					//msleep(30);
					tp_homekey_control(0);
	...
	if(intr_status & CSL_CAM_INT_FRAME_END) {
		...
		if(c->mode == CAM_STREAM) {
...
#endif
			if(atomic_read(&shutdown_req)==1) {
				csl_cam_rx_stop(c->hdl);
				up(&fe_sem);
				atomic_set(&shutdown_req,0);
				printk("StopH!!\n");
				return IRQ_HANDLED;
			}
			if((c->main.fps == 15) && (c->cslCamIntfCfg.intf == CSL_CAM_INTF_CSI)) {
				...
				if(is_dma_started) {
					DMADRV_Force_Shutdown_Channel(c->cur_dma_chan);
					is_dma_started = 0;
					printk("\n#### PANIC1 case in Camera####\n");
					return;
				}
				cur_dma_chan = mem_mem_dma(c->gCurrData.busAddress,(dma_addr_t)image_addr,(c->main.size_window.end_pixel * c->main.size_window.end_line * 2));
				if(cur_dma_chan > 0)
					c->cur_dma_chan = cur_dma_chan;
			}
			/* And then do what ?? */
			...
			if(c->sCaptureFrameCountdown <= 0) {
				if( (fs != 0) && (fe != 0)) {
					...
					if((image_addr == picture_storage_address) && ((intr_status & CSL_CAM_INT_FRAME_ERROR) == 0)) {
						...
						printk("### RDR3 is 0x%x... RS0 is 0x%x ### stop host\n", rdr3_cnt, raw_intr_status);
...
static void mem_mem_dma_isr(DMADRV_CALLBACK_STATUS_t status)
{
	...
	if(status == DMADRV_CALLBACK_OK)
	{
		...
		printk("DMA ISR returned failure\n");
}
...
static int mem_mem_dma(dma_addr_t dst_addr, dma_addr_t src_addr, int dma_tx_size)
{
	...
	if(src_addr == (dma_addr_t)NULL)
		{
			pr_debug("SRC addr is zero, but no guy to receive\n");
			wakeup_push_queue(&c->rd_Q, &c->gCurrData);
			pull_queue(&c->wr_Q, &c->gCurrData);
			return -1;
	...
	case CAM_IOCTL_GET_CAPTURE_TYPE:
		{
			...
			cap_fmt = Cam_Capture_YUV;	//Cam_Capture_JPEG;  hanwei chg for chenggong merge patch lead to system halted.
#else
...
static int process_frame(CamSensorSelect_t sensor)
{
	...
	/* Block waiting to be signalled by either VSYNC ISR or camera disable */
		ret =  wait_event_interruptible_timeout(gDataReadyQ, c->still_ready, msecs_to_jiffies(1500));
#ifdef CAM_CORRUPTION_CONCURRENCY
			writel(0x3f0f, io_p2v(BCM21553_MLARB_BASE + 0x100)); //BMARBL_MACONF0 
#endif
		printk("FEsync.. Ds Host\n");
		
		sh_mem_addr = ioremap_nocache(CP_SHARED_MEM, CP_SHARED_MEM_SIZE);

		if(readl(sh_mem_addr) != 0x00)
			printk("**MM: before HD in FEsync **** \n");

		csl_cam_rx_stop(c->hdl);
		printk("FEsync..Ds Intr\n");
		disable_irq(c->cam_irq);
		printk("FEsync..Ds Sensor\n");
		
		if(readl(sh_mem_addr) != 0x00)
			printk("**MM: before SD in FEsync **** \n");

		c->sens_m->DRV_DisableCapture(sensor);

		if(readl(sh_mem_addr) != 0x00)
			printk("**MM: after SD in FEsync **** \n");
		iounmap(sh_mem_addr);
 ...
 * The disable method wakes up with gProcessFrameRunning = 0 which means exit */
			...
			if(c->main.format != CamDataFmtJPEG){
				...
				c->sCaptureRawSize=c->sens_m->DRV_GetJpegSize(sensor,NULL);
			...
			printk(KERN_ERR"Timeout trying to capture YUV .. returning 0 length\n");


			u32 *src=(u32 *)cam_g->cam_buf.virt ;
			int i;

			for(i=0;i<(SZ_BUFFER/4);i++){
				*(src+i)=0x80058005;
			}
			
			c->sCaptureJpegSize = (c->main.size_window.end_pixel*c->main.size_window.end_line*2);
			c->sCaptureRawSize = c->sCaptureJpegSize;
			break;
		}
	}
	...
	ret = down_timeout(&queue->Sem,msecs_to_jiffies(500));
		...
		if(atomic_read(&shutdown_req)==1 ) {
			...
			atomic_set(&shutdown_req,0);
                ...
                writel(0x3f0f, io_p2v(BCM21553_MLARB_BASE + 0x100)); //BMARBL_MACONF0
#endif
				atomic_add(1, &c->preview_timeout);
				if(atomic_read(&c->preview_timeout)>=7){
					printk("home key pressed for preview timeout \n");	
					tp_homekey_control(1);
					//msleep(30);
					tp_homekey_control(0);

					atomic_set(&c->preview_timeout,0);
				}
...
//hanwei@wind-mobi.com chg 2011.11.04 end
	...
	//			cslCamBuffer1.start_addr = cam_g->cam_buf.phy + (4*1024*1024);
...

		{
			...
			//memset(cam_g->cam_buf.virt, 0xAB, SZ_BUFFER);  just for capture huaping debug
		}
		...
		if(cam_g->saved_mem){
			memcpy_fromio(cam_g->saved_mem,sh_mem_addr, CP_SHARED_MEM_SIZE );
		}
			
		if(csl_cam_rx_start(c->hdl))
			pr_debug("Unable to start RX in capture mode\n");
		...
		if(readl(sh_mem_addr) != 0x00){
			printk("**MM: we need to resore memory: before SE  **** \n");
			memcpy_toio(sh_mem_addr,cam_g->saved_mem, CP_SHARED_MEM_SIZE );
		}
		...
		if(readl(sh_mem_addr) != 0x00){
			printk("**MM: we need to resore memory :after SE **** \n");
			memcpy_toio(sh_mem_addr,cam_g->saved_mem, CP_SHARED_MEM_SIZE );
		}
			
		iounmap(sh_mem_addr);
		
		printk("Completed Capture settings\n");
		c->mode = CAM_STILL;
		c->prev.tv.sec = 0;
		c->prev.tv.nsec = 0;
		/*
		...
		/* 2 bpp */
		size = (sensor_size.resX * sensor_size.resY * 2 * 4);
		...
		//printk(KERN_ERR"+++++++ c->cslCamIntfCfg.intf  %d \n", c->cslCamIntfCfg.intf);

	/*del-s by hanwei 20120329 here just for debug*/
	#if 0 
		if (c->sens_m->DRV_Wakeup(cam_g->curr)) {
    	printk(KERN_ERR "Failed to init the sensor\n");
		//rc = -EFAULT;
	...
	#endif
	/*del-e by hanwei 20120329 here just for debug*/
		sh_mem_addr = ioremap_nocache(CP_SHARED_MEM, CP_SHARED_MEM_SIZE);
		printk("En Host for Preview\n");

		if(readl(sh_mem_addr) != 0x00)
			printk("**MM: before HE in preview **** \n");
	
		
		if(csl_cam_rx_start(c->hdl))
			pr_debug("Unable to start RX\n");
		//enable_irq(c->cam_irq);	//modify by zhaoqiang
		printk("En Intr for preview \n");
		enable_irq(c->cam_irq);
		printk("En sensor for preview\n");

		if(readl(sh_mem_addr) != 0x00)
			printk("**MM: before SE in preview**** \n");
...
/* SetParm IOCTL would have populated the CAM_PARM_t structure */
	...
	//	down(&fe_sem);
		...
		* a max of 100msec delay to work for 10 to 30 fps */
		atomic_set(&shutdown_req,1);
		down_timeout(&fe_sem,msecs_to_jiffies(500));
...
static int cam_open(struct inode *inode, struct file *file)
{
	...
	//hanwei-s Init camera over here 20120329	
	...
	//hanwei-d Init camera over here 20120329	
...
//#endif
	...
	cam_g->saved_mem= kmalloc(CP_SHARED_MEM_SIZE,GFP_KERNEL);
	if (!cam_g->saved_mem)
		printk("camera:  can not allocate saved_mem for  mem restore \n");

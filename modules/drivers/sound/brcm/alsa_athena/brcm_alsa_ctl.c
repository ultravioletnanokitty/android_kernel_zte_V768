...
//	DEBUG("\n%lx:GetCtrlInfo ctrl->private_value=%d",jiffies, kcontrol->private_value);
	...
	switch(kcontrol->private_value)
	{
		...
		case BRCM_CTL_HAC_MODE:
			...
			uinfo->value.integer.step = 1;
		break;
		case BRCM_CTL_TTY_MODE:
				DEBUG("\BRCM_CTL_TTY_MODE");
			uinfo->type			     = SNDRV_CTL_ELEM_TYPE_INTEGER;
			uinfo->count			 = 1;	   // TTY OFF, TTY FULL, HCO , VCO
			uinfo->value.integer.min = 0;
...
//				audvoc_configure_Volume(0, 0x1FF) ;
		...
		case BRCM_CTL_TTY_MODE:
			{   
                BRCM_AUDIO_Param_Tty_Mode_t parm_tty_mode;

                parm_tty_mode.tty_mode = (int)ucontrol->value.integer.value[0];
                 // Send the tty mode command
                AUDIO_Ctrl_Trigger(ACTON_TTY_MODE,&parm_tty_mode,NULL,1);
            }
            break;			
                default:
                    break;
...
static struct snd_kcontrol_new gpSndCtrls[] __devinitdata =
{
	...
	BRCM_MIXER_CTRL(gpstrCtrlName[BRCM_CTL_TTY_MODE], 0, BRCM_CTL_TTY_MODE)

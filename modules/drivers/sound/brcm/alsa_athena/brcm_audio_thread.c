...
static UInt32 tty_mode = 0;//TTY_OFF
/* ---- Data structure  ------------------------------------------------- */
...
typedef	struct	_TMsgBrcmAudioCtrl
{
	...
	void								*pCallBack;
    int                                 block;
    //liutao@wind-mobi.com 2012-2-22 begin
    //fix csp 500353 cp crash
    //liubin@wind-mobi.com 2012-2-22 review
	int 								dummy;	//kfifo requires msg length is pow of 2, so add an dummy int to make the msg length to be 32 bytes
    //liutao@wind-mobi.com 2012-2-22 end
...
#define	KFIFO_SIZE		(8*sizeof(TMsgAudioCtrl))
    //liutao@wind-mobi.com 2012-2-22 end
	...
	)
{
    ...
    switch (action_code)
    {
        ...
        case ACTION_AUD_StopRecord:
        {
			...
			if(AUDIO_Policy_RemoveModeFromQueue(BRCM_STATE_RECORD, &prev_mode, &prev_app))
				AUDCTRL_SaveAudioModeFlag(prev_mode,prev_app);
            AUDIO_Policy_RestoreState(BRCM_STATE_RECORD);
        }
	break;
	...
	case ACTION_AUD_OpenRecord:
        ...
        case ACTON_VOICECALL_START:
        {
			...
			int telephony_mic, telephony_speaker;
			...
			telephony_mic = sgTableIDChannelOfVoiceCallDev[telephony_stream_number-VOICE_CALL_SUB_DEVICE].mic;
			telephony_speaker = sgTableIDChannelOfVoiceCallDev[telephony_stream_number-VOICE_CALL_SUB_DEVICE].speaker;
			...
			// If audio to be routed to headset check if TTY mode is enabled
			if(telephony_stream_number-VOICE_CALL_SUB_DEVICE == 2)
			{
				// if TTY mode is on set the mode accordingly
				if(tty_mode == TTY_FULL)
					audio_mode = AUDIO_MODE_TTY;
				else if (tty_mode == TTY_HCO)
					audio_mode = AUDIO_MODE_TTY_HCO;
				else if (tty_mode == TTY_VCO)
					audio_mode = AUDIO_MODE_TTY_VCO;

				// Route the data based on TTY mode
				if(tty_mode == TTY_FULL)
				{
					telephony_mic = AUDCTRL_MIC_AUX;
					telephony_speaker  = AUDCTRL_SPK_HEADSET;
				}
				else if (tty_mode == TTY_HCO)
				{
					telephony_mic = AUDCTRL_MIC_AUX;
					telephony_speaker = AUDCTRL_SPK_HANDSET;
#if 0					
					// if we are in headset mode change to earpiece
					// else route it to loud speaker
					if(telephony_speaker == AUDCTRL_SPK_HEADSET)
						telephony_speaker = AUDCTRL_SPK_HANDSET;
					else if (telephony_speaker == AUDCTRL_SPK_LOUDSPK)
						telephony_speaker = AUDCTRL_SPK_LOUDSPK;
#endif						
				}
				else if (tty_mode == TTY_VCO)
				{
					telephony_mic = AUDCTRL_MIC_MAIN;
					telephony_speaker = AUDCTRL_SPK_HEADSET;
				}
			}
            ...
            AUDCTRL_EnableTelephony(AUDIO_HW_VOICE_IN,AUDIO_HW_VOICE_OUT,telephony_mic,telephony_speaker);
            telephony_started = 1;
        ...
        case ACTON_VOICECALL_STOP:
        {
			...
			if(AUDIO_Policy_RemoveModeFromQueue(BRCM_STATE_INCALL, &prev_mode, &prev_app))
				AUDCTRL_SaveAudioModeFlag( prev_mode,prev_app);
            AUDIO_Policy_RestoreState(BRCM_STATE_INCALL);
        }
        break;
        ...
        case ACTON_VOICECALL_UPDATE:
        {
			...
			// If TTY is enabled update mode accordingly, if we are routing to headset
			if(telephony_stream_number-VOICE_CALL_SUB_DEVICE == 2)
			{
				if(tty_mode == TTY_FULL)
					audio_mode = AUDIO_MODE_TTY;
...
*/						
        ...
        case ACTON_FM_STOP:
        {
			...
			if(AUDIO_Policy_RemoveModeFromQueue(BRCM_STATE_FM, &prev_mode, &prev_app))
				AUDCTRL_SaveAudioModeFlag( prev_mode,prev_app);
            AUDIO_Policy_RestoreState(BRCM_STATE_FM);
        }
        break;
        case  ACTON_ROUTE:
            {
        ...
        case ACTON_TTY_MODE:
            {
                // update the TTY mode
                BRCM_AUDIO_Param_Tty_Mode_t* parm_tty_mode = (BRCM_AUDIO_Param_Tty_Mode_t*)arg_param;
                // update the TTY mode
                tty_mode = parm_tty_mode->tty_mode;
...
int AUDIO_BRCMAudioParamSize(BRCM_AUDIO_ACTION_en_t action_code)
	{
		...
		switch(action_code)
		{
           ...
           case ACTON_HAC_MODE:
			...
			case ACTON_TTY_MODE:
            {
                ret =  sizeof(BRCM_AUDIO_Param_Tty_Mode_t);
            }
            break;		
			default:
			{

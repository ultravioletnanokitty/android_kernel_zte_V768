...
//and rename it AUD_SPEAKER_t
	...
	Log_DebugPrintf(LOGID_AUDIO,"powerOnExternalAmp1, speaker = %d, IHF_IsOn= %d, HS_IsOn = %d, use=%d, usage_flag=%d\n", speaker, IHF_IsOn, HS_IsOn, use, usage_flag);
	Log_DebugPrintf(LOGID_AUDIO,"powerOnExternalAmp1 (HS on), telephonyUseHS = %d, voiceUseHS= %d, audioUseHS= %d, polyUseHS = %d\n", telephonyUseHS, voiceUseHS, audioUseHS, polyUseHS);
	Log_DebugPrintf(LOGID_AUDIO,"powerOnExternalAmp1 (IHF on), telephonyUseIHF = %d, voiceUseIHF= %d, audioUseIHF= %d, polyUseIHF = %d \n", telephonyUseIHF, voiceUseIHF, audioUseIHF, polyUseIHF);
	switch(speaker)
	{
		case AUDCTRL_SPK_HEADSET:
	...
	else
	{
		...
		if ((mode == AUDIO_MODE_TTY) || (mode == AUDIO_MODE_TTY_HCO) || (mode == AUDIO_MODE_TTY_VCO))
		{
			// use the gain parameters from TTY modes
	...
	else
	{
		...
		IHF_IsOn = TRUE;
//songjinguo@wind-mobi.com 20120421 start
//fix bug camera focus sound only one times;
//xiaocui@wind-mobi.com 2012.06.20 delete
//this code caused bug 13200,boot sound mixed with a plosive
		//msleep(50);
//songjinguo@wind-mobi.com 20120421 end
	}
#endif
}
...
//and rename it AUD_SPEAKER_t
	...
	Log_DebugPrintf(LOGID_AUDIO,"powerOnExternalAmp2, speaker = %d, IHF_IsOn= %d, HS_IsOn = %d, use=%d, usage_flag=%d\n", speaker, IHF_IsOn, HS_IsOn, use, usage_flag);
